...
| `destroy-lifecycle.cy.ts`   | Editor destroy closes only its own popover; a foreign editor's destroy leaves an open popover mounted; `createPopover` is a one-shot (`show()` after `hide()`/`destroy()` no-ops, controller stays idle)                                                           |
| `xss-guards.cy.ts`          | Dangerous schemes blocked at parse, input rule, click, serialize; control chars can't smuggle `javascript:`                                                                                                                                                        |
| `nav-guards.cy.ts`          | Middle-click (`auxclick`) and the read-only click fallback (`?readonly=on&popover=none`) route through the navigation gate with `noopener,noreferrer`                                                                                                              |
| `touch-tap.cy.ts`           | Phone taps: `touchend` alone opens the preview and consumes the tap (isolated synthetic event + trusted `realTouch` sequence); read-only tap hits the gated fallback                                                                                               |
...
| `markdown-round-trip.cy.ts` | `@tiptap/markdown` full round-trip for the hyperlink mark; re-import of exported markdown; canonical hrefs; escaping; nested marks; import XSS; input-rule parity; plus the `getHTML()` → `setContent()` HTML round-trip, where a stored `target` must not survive |
