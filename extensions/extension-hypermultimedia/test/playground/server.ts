/**
 * Clean-room playground server for @docs.plus/extension-hypermultimedia.
 *
 * Bun-native replacement for a Vite-style dev server. Using
 * `Bun.serve({ routes: { '/': index } })` with an HTML import means Bun
 * bundles the referenced `<script>` tag on demand — no config, no plugins.
 * The editor loads the extension's published `dist/` via the exports map,
 * so the Cypress suite exercises the same artifact a fresh npm consumer gets.
 */
