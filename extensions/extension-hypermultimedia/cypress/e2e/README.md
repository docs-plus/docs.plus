...
| **Serialization**  | `serialization/serialize-media.cy.ts`, `serialization/html-round-trip.cy.ts` | `getHTML()` never throws with video/audio nodes; setContent round-trip for all eight nodes; toolbar Copy serializes cleanly                           |
...
| Lifecycle   | `lifecycle/editor-destroy.cy.ts` (destroy with chrome open tears down everything; re-init on the same element works), `lifecycle/x-embed-mount.cy.ts` (X mount abort + fluid height after ready) |
...
| Security    | `security/loom-parse-s1.cy.ts`, `security/x-parse-s2.cy.ts` (runs with `?blockquote=off` so X's parse rule wins), `security/x-oembed-s3.cy.ts`, `security/x-oembed-fallback.cy.ts`               |
