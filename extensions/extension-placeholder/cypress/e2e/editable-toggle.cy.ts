...
// Read-only handling: showOnlyWhenEditable hides the placeholder in read-only,
...
describe('placeholder — editable toggle', () => {
  ...
  it('hides the placeholder on setEditable(false) and restores it on setEditable(true)', () => {
    cy.visitPlayground()
    cy.get('#editor [data-placeholder]').should('exist')
