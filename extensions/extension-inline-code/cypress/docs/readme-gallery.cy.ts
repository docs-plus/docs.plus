...
const DOC_WITH_CODE = '<p>Call the <code>render()</code> helper to paint the view.</p>'

describe('README screenshot gallery', { testIsolation: false }, () => {
  beforeEach(() => {
    prepareReadmeGalleryViewport()
    cy.visitPlayground()
    cy.setEditorContent(DOC_WITH_CODE)
    cy.get('#editor code').should('be.visible')
  })

  for (const theme of ['light', 'dark'] as const) {
    it(`captures inline code mark (${theme} theme)`, () => {
