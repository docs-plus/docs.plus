...
| active / selected (toolbar + rows) | **`is-active` pattern**: `color-mix(primary 10%)` bg + `text-primary` (`_toolbar.scss`); TOC rows: daisyUI `menu-active` / `menu-focus` (see §Table of contents); nav items: `variant="primary"`                                                                                                                                                                                                                                                                                                                                           |
...
| idle / hover / unread  | desktop idle/hover from daisyUI `.menu`; chat icon `toc__chat-icon text-base-content/60 hover:text-primary` (mobile header same `/60`); unread swaps icon for `UnreadBadge size=sm variant=error` inside the same chat slot                                                                                                                                     |
| active (doc chat open) | desktop `menu-active` on the header `TocRow`; mobile icon `toc__chat-icon--active` / `--color-docsy` (never `text-accent`)                                                                                                                                                                                                                                      |
...
| hover / pressed              | daisyUI `.menu` hover / `:active` on `.toc__row`; grip / fold / chat always visible                                                                                                                                                                                                                                                                                                              |
| active / focused / menu-open | chat open → `menu-active` on `.toc__row`; spy → `menu-focus` when not also chat-open; icon `toc__chat-icon--active`; context-menu-active `color-mix(primary 15%)`                                                                                                                                                                                                                                |
| unread / presence            | `UnreadBadge` replaces icon in the chat slot; `AvatarStack` desktop only (in-flow before chat); mobile presence fork: chat + unread only                                                                                                                                                                                                                                                         |
...
| State                   | Recipe                                                                                                                                       |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| frame                   | grip: 1.25rem square, `border-radius: var(--radius-field)`; always visible; hover primary ink + 8% wash                                      |
...
| State               | Recipe                                                                                                                            |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| edge `ring`         | default solo: container `avatar` + `SIZE_CLASSES` + `rounded-full bg-base-200` + `avatarEdgeClass('ring')` + inset keyline on img |
| edge `paper`/`well` | stack cutouts: `avatarEdgeClass` only (no inset keyline); stacks set edge via `stackSurfaceToEdge(surface)`                       |
...
| overflow +N  | same `avatarEdgeClass(stackSurfaceToEdge(surface))` + `bg-base-100` + inner `bg-neutral text-neutral-content font-semibold`                         |
