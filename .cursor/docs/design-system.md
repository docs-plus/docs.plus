...
| State                  | Recipe                                                                                                                                                                                                                                                                                                                                                                |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| frame                  | desktop `border-base-300 bg-[var(--pad-well)] z-30 border-b pt-2 pb-1` + `paddingLeft: var(--toc-list-inset)`, row `toc__header-row rounded-field py-1 pr-3` + `--toc-row-pad-start` (idle `0.5rem` / active `1rem`), title `text-base font-semibold`; mobile `border-base-300 bg-base-100 z-30 border-b px-4 py-3` + trigger `toc__chat-trigger size-8 rounded-full` |
| idle / hover / unread  | desktop header hover `color-mix(base-content 8%, pad-well)` when not active; chat icon `toc__chat-icon text-base-content/60 hover:text-primary`; mobile idle `text-base-content/40`, fold/chat `opacity:1 !important`; unread swaps icon for `UnreadBadge size=sm variant=error` inside the same chat slot                                                            |
| active (doc chat open) | desktop `toc__active-border bg-base-300` + inset `::after` (docsy) with pad clear via `--toc-row-pad-start: 1rem`; mobile icon `toc__chat-icon--active` / `--color-docsy` (never `text-accent`)                                                                                                                                                                       |

### TOC rows (TocRow + TocItemDesktop + TocRowTrail + TocItemMobile + fold) — both

control — `components/toc/TocRow.tsx`, `TocItemDesktop.tsx`, `TocRowTrail.tsx`, `TocItemMobile.tsx` (state SCSS `styles/components/_tableOfContents.scss`)

| State                        | Recipe                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| frame                        | shared `TocRow` (`level` + `title` string): `toc__row rounded-field pr-3 min-h-8` (mobile `min-h-11`); pad via `--toc-row-pad-start` (idle `8px`, active/spy `1rem`, nest `calc(8px + d×nest)` — no `!important` war); nest `[data-depth=d]` **pull** (`margin-left: -d×nest`, width + gutter + nest); daisyUI `.toc__list.menu :where(li ul)::before` under `.tiptap__toc` — stroke `base-content` ~24% mix, full height; trail `right: var(--toc-trail-inset)` `flex-row-reverse`; vars `--toc-scrollbar-gutter:11px; --toc-list-inset:16px; --toc-trail-inset:12px; --toc-nest-indent:16px` |
| type ladder                  | `toc__link--level-{1–6}`: H1 `14px/600`; H2–3 `14px/500`; H4–6 `13px/400` at `base-content` 82% mix; titles **wrap** (`whitespace-normal text-pretty`) — never truncate / `pre-line`                                                                                                                                                                                                                                                                                                                                                                                                           |
| hover / pressed              | hover `color-mix(base-content 8%, pad-well)` (not `base-300`); reveals grip + fold + chat icon; unread/presence never opacity-gated; `:active` `base-content` 10%                                                                                                                                                                                                                                                                                                                                                                                                                              |
| active / focused / menu-open | active `toc__active-border bg-base-300` + inset `::after` (4px); spy `::before` same inset geometry (static); spy ∩ active suppresses `::before`; both retune `--toc-row-pad-start` to `1rem` at depth 0 (nest depths already clear); icon `toc__chat-icon--active`; context-menu-active `color-mix(primary 15%)`                                                                                                                                                                                                                                                                              |
| unread / presence            | `UnreadBadge` replaces icon in the same 24×24 slot; `AvatarStack` desktop only (mobile presence fork: chat + unread only)                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| fold caret                   | desktop hover-only (opacity 0→1); mobile/TocModal always on; rotate 160ms; focus-visible `var(--focus-ring-soft)`                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| State                   | Recipe                                                                                                                                       |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| frame                   | grip: 1.25rem square, `border-radius: var(--radius-field)`; opacity 0 until row hover / `:focus-visible`; hover primary ink + 8% wash        |
...
| loader       | `AvatarStackLoader` takes the same `surface` + size (Participants: `size=sm`); TOC width reserve = `tocPresenceReservePx`                           |
