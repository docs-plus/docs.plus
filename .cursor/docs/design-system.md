...
as a color-swap inside a shared structure (see `--pad-sheet-shadow`).
...
| **L2 · modal**                                 | L1 frame (`modalPanelFrameClassName` / `modalPanelClassName`, `ui/Dialog.tsx`; panel clips with `overflow-clip` so scroll-into-view can't scroll the card box) + `modalBackdropClassName` scrim (`--modal-scrim` + `motion-safe:backdrop-blur-sm`); bottom sheets share the scrim, **no blur**. **Mobile takeover arm** — `ModalContent mobileTakeover` (`modalPanelTakeoverClassName`): below `md` the card fills the visual viewport (`--visual-viewport-height`, `100dvh` fallback) full-bleed — no radius/border/shadow, `env(safe-area-inset-top)` pad, positioner `max-md:p-0` — same scrim and 180/150 motion tier; for full-screen multi-section hubs (HIG/M3), never for single-purpose tasks (those stay sheets/dialogs) | Dialogs, GlobalDialog, Share/Profile modals, mobile bottom sheets (chat excluded — it is L0 docked), Settings mobile takeover              |
| **Tooltip**                                    | `--radius-field` + hairline `0 2px 4px` base-content 10% (`.Popover`, `_popover.scss`) — deliberately lighter than L1                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Tooltips only                                                                                                                              |
| **Deliberate exceptions** (do not "normalize") | TOC drag card `0 8px 24px` + hard-edge deck stack; crinkle paper-fold shadows (`_heading-fold.scss`); bottom-anchored mobile toolbars cast **upward** (`_toolbar.scss`, `_blocks.scss`); focus/pressed rings expressed as `box-shadow: 0 0 0 1-2px` primary mixes                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | drag feedback, skeuomorph fold, mobile bars                                                                                                |
...
| active / selected (toolbar + rows) | **`is-active` pattern**: `color-mix(primary 10%)` bg + `text-primary` (`_toolbar.scss`); TOC **`menu-active`**: opaque soft brand (`--menu-active-bg` = `color-mix(primary 14%, var(--pad-well))`, `--menu-active-fg` = `primary`, scoped to `.toc__list.menu` in `_daisyui.scss` — never `transparent` mix, never charcoal `neutral`); **`menu-focus`** stays `base-300` (scroll-spy only); nav items: `variant="primary"`                                                                                                                |
| context-menu-active                | `color-mix(primary 15%)` on TOC rows, `8%` on message cards — row keeps highlight while its menu is open                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| unread                             | `UnreadBadge` (`bg-error text-error-content rounded-full`) + `animate-badge-entry` + `RollingNumber`; count via `resolveUnreadCount` (`utils/unreadDisplay.ts`). TOC/header React-only; ProseMirror `.ha-chat-btn` widgets via `syncHeadingWidgetUnread` + `[data-unread-count]` CSS                                                                                                                                                                                                                                                       |
...
| buttons        | share `btn btn-primary` (authed adds `btn-outline`); history/bell `btn btn-ghost btn-circle` + `text-base-content/70`, bell unread `UnreadBadge size=sm variant=error absolute -top-1 -right-2`; avatar `btn btn-ghost btn-circle btn-lg border-0 p-0` wrapping `Avatar` (default frame); sign-in `btn btn-neutral` |
| presence stack | `PresentUsers` → `AvatarStack size=sm surface="outline"`; edge/size geometry from `utils/avatarStackGeometry.ts` (`stackSurfaceToEdge` → `avatarEdgeClass`); faces `bg-base-100 animate-badge-entry`; +N same edge + `bg-neutral text-neutral-content font-semibold`                                                |
...
| filter chip    | `border-info/30 bg-info/10 text-[var(--info-ink)] m-0.5 rounded-field border py-0.5 pr-0.5 pl-2 text-xs font-medium motion-safe:animate-[doc-region-in_180ms_ease-out_both]` (chip ink is `--info-ink`, not raw `info` — plain `info` fails AA on the `info/10` ground); remove-x `inline-flex size-11 min-h-11 min-w-11 rounded-full opacity-70 hover:opacity-100` (44px touch target); Reset `btn btn-ghost btn-xs` — desktop hover-reveal `opacity-0 group-hover:opacity-100`, mobile always visible; sheet FilterPanel also shows Reset beside chips |
...
docked — `components/toc/TocHeader.tsx` (desktop mounted as first `<li>` in `TocDesktop`)

| State                  | Recipe                                                                                                                                                                                                                                                                                                                                                          |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| frame                  | desktop sticky first `li.toc__header` inside `ul.toc__list.menu` (`top:0`, `bg-[var(--pad-well)]`, `border-b`, z-30); row is shared `TocRow` (`toc__header-row`) + `TocRowTrail`; title `text-base font-semibold`; mobile TocModal stays outside menu — `border-base-300 bg-base-100 z-30 border-b px-4 py-3` + trigger `toc__chat-trigger size-8 rounded-full` |
| idle / hover / unread  | desktop idle/hover from daisyUI `.menu`; chat/fold/grip ink from `_tableOfContents.scss` (muted `base-content`, hover `primary`); desktop grip + chat opacity-0 until `li` hover (unread `.badge`, menu-active/focus, focus-visible stay on); mobile always shows chat; unread swaps icon for `UnreadBadge size=sm variant=error`                               |
| active (doc chat open) | desktop `menu-active` on the header `TocRow` — opaque soft primary on `--pad-well` (`--menu-active-*`); grip/fold/chat inherit row `primary` (not `primary-content` — that was for the old solid-neutral pill); mobile icon `toc__chat-icon--active` / `--color-docsy` only off the primary row (never `text-accent`)                                           |

### TOC rows (TocRow + TocItemBody + TocItemDesktop + TocRowTrail + fold) — both

control — `components/toc/TocRow.tsx`, `TocItemBody.tsx`, `TocItemDesktop.tsx` (sortable shell), `TocRowTrail.tsx` (state SCSS `styles/components/_tableOfContents.scss`)

| State                        | Recipe                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| frame                        | shared `TocRow` inside `ul.toc__list.menu` (after sticky `li.toc__header`): daisyUI menu owns item grid / `px-3 py-1.5` / type / `rounded-field` / hover / nest rails (stock); three children — leading (grip/fold) \| title `<a>` \| `TocRowTrail` end slot; `align-items: flex-start` only (wrap titles — menu defaults to center); `min-h-8` (mobile `min-h-11`); list shell `padding: 0 6px`                                                                                                                                                                                                                                                                                           |
| title                        | one weight/size from `.menu`; titles **wrap** (`text-pretty`) — never truncate; hierarchy via nest only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| hover / pressed              | row hover = focus family but darker (`color-mix(base-content 14%, base-300)`); `menu-focus` stays solid `base-300`; fold always visible; desktop grip + chat reveal on `li` hover (mobile chat always on; grip desktop-only)                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| active / focused / menu-open | chat open → soft-primary `menu-active`; spy → `menu-focus` (`base-300` — not brand); grip chrome unified (`base-300` border + micro shadow); grip fill mirrors row (idle `base-100` / focus `base-300` / hover darker mix / active `--menu-active-bg` primary mix); fold/chat inherit row `primary` ink; context-menu-active `color-mix(primary 15%)`                                                                                                                                                                                                                                                                                                                                      |
| unread / presence            | `UnreadBadge` replaces icon in the chat slot; desktop presence is in-flow `AvatarStack anchor="right"` right of the chat trigger (fixed right edge, grows left, `translate-x-3` to the column inner edge); `.toc__scroll` is column-width with a stable gutter so the scrollbar stays inside the wrapper. **Beyond-column overhang is settled-rejected** (do not re-attempt): `overflow: visible` breaks scroll, a per-row JS overlay reflows every scroll frame, CSS anchor positioning isn't cross-browser (Firefox 2026) — presence stays in-panel like Google Docs / Figma / Slack. TOC `z-[42]` / sash `z-[41]` under Dialog/Popover `z-50`; mobile presence fork: chat + unread only |
| drag states                  | ghosted dashed `base-300` + `border-radius: var(--radius-field)`; list `.is-dragging` hides trails                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| empty state                  | both surfaces via `TocEmptyState`: `Add headings to build an outline.` + `AppendHeadingButton className="mt-3"`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| fold caret                   | plain `<button>` chevron (not `btn-ghost`); always visible; rotate 160ms; focus-visible `var(--focus-ring-soft)`; drag overlay SCSS lives in `_tocDrag.scss`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| mobile rows                  | same `TocRow` + nested `toc__children` under `menu`; fold ~44px hit; chat via `TocRowTrail`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| State                   | Recipe                                                                                                                                                                                                                                                                            |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| frame                   | grip: ~1.15×1.42rem, `border-radius: var(--radius-field)`; one chrome family — always `base-300` border + micro black shadow; fill mirrors row: idle `base-100`, focus `base-300`, hover darker mix, active `--menu-active-bg/fg`; desktop hover-reveal 120ms opacity + scale pop |
| overlay card            | `border-radius: var(--radius-field)`; light `0 8px 24px color-mix(black 12%)`; dark/HC `28%` — deliberate drag lift (not `--shadow-overlay`)                                                                                                                                      |
| deck + drop indicator   | `.toc-stack-indicator[data-count=1–3]` layered base-200/base-300 shadows; drop line 2px + circle `::before`                                                                                                                                                                       |
| level picker + chips    | `.toc-drag-levels { border-base-300; border-radius: var(--radius-box); box-shadow: var(--shadow-overlay) }` — L1 floating                                                                                                                                                         |
| level badge (drag-only) | hidden until `.is-dragging`; docsy fill + primary-content ink                                                                                                                                                                                                                     |
...
| empty feed               | Virtuoso `EmptyPlaceholder` → `MessagesEmptyState`: `badge badge-soft badge-primary` “No messages yet!” (soft brand — not `badge-neutral`)                                                                                             |

### ChatroomToolbar (desktop header) + ChatroomPaneLayout header — both

docked — `components/chatroom/components/ChatroomToolbar/ChatroomToolbar.tsx`, `…/Chatroom/layouts/ChatroomPaneLayout.tsx`
...
| cluster buttons | `Button ghost sm square` + `text-base-content/60 hover:text-base-content hover:bg-base-300 focus-visible:ring-primary/30 focus-visible:ring-2 focus-visible:outline-none`; media-filter pressed `text-primary bg-primary/10`; participants loading `AvatarStackLoader size=sm surface=paper repeat=2` |
...
card — `components/chatroom/components/MessageCard/components/MessageContent/components/` (+
`components/chatroom/components/ChatMediaGallery/`; feed theme
`components/chatroom/utils/messageMediaTheme.ts`)
...
| image collage / tile | mosaic grid `bg-base-300/40 gap-px rounded-field`; overflow chip `bg-base-content/60 text-base-100 rounded-selector`; tile shell `rounded-field` / single video `rounded-box`; spoiler `scale-110 blur-xl` + `FeedSpoilerRevealOverlay` (`bg-base-content/35 text-base-100 text-xs`); loading `skeleton`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| video / audio / file | video `rounded-box border bg-black`; expand `btn btn-circle btn-xs bg-base-content/60 text-base-100 border-0 sm:opacity-0 sm:group-hover/video:opacity-100` (audio mirrors, `group-hover/audio`); spoiler gates poster/lone video/audio before `openGallery`; audio `rounded-field border px-3 py-2.5` + disc `size-10 rounded-full`; file `hover:bg-base-200/50 rounded-field border px-3 py-2.5`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| unavailable          | `bg-base-200 text-base-content/70 p-3 text-xs` + retry `text-primary hover:underline focus-visible:underline`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| lightbox (modal L2)  | Full-bleed `fixed inset-0 z-[100] h-[100dvh]` + `modalBackdropHeavyClassName`; black stage (no `modalPanelFrameClassName`); Discord-parity CSS vars scoped on root (`--gallery-panel-*` / `--gallery-text-*` — **Off-system** media-overlay ink, not daisyUI); author block top-left; action pill + close top-right (`rounded-box` / `rounded-field`); counter pill via `--gallery-counter-top`; caption bottom; nav chevrons `rounded-full` without drop shadow; spoiler uses `GallerySpoilerRevealOverlay` (black/white). **Mobile overflow-first** (`galleryToolbarModel.ts`): one action list with `pill` / `overflowPrefix` / `overflow` surfaces — author `flex-1` truncate; pill = download + ⋯; zoom / reply / open on `overflowPrefix`; close separate. **⋯ menu:** desktop anchored dropdown; mobile bottom sheet portaled to `document.body` (`z-[120]`, gallery tokens on portal root — pill `backdrop-blur` must not host `fixed`) |

> Off-system: lightbox is a bespoke `z-[100]` media viewer — heavy scrim, full-bleed black stage,
> Discord hex panel tokens (not `base-*`). On-image controls stay white/black. Inline overflow menu
> (not portaled `Popover`) so dismiss does not fight the modal.
...
| State                       | Recipe                                                                                                                                                                                                                                                                                                                                              |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| frame                       | desktop `composer-bar border-base-300 bg-base-200 mb-2 rounded-field border`; mobile `composer-bar--mobile border-base-300 bg-base-200 border-t` (no radius — dock step off feed paper `bg-base-100`); input row `gap-1.5 px-3 py-2` (mobile `min-h-11`); ProseMirror line 2rem desktop / 2.75rem mobile; action targets 44px mobile / 32px desktop |
...
| send / mic                | same ghost control species as + / emoji: send (canSend) `variant=ghost text-primary` + send icon `stroke-[1.75]` (not a filled primary disc); mic ghost + tooltip; recording adds `text-error` aria-pressed; preview renders null (bar owns UI); mobile `size-11` / desktop `size-8` shared hit target so mic↔send does not jump                            |
...
| State         | Recipe                                                                                                                                                                                                                                                                                                                                                            |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| frame         | `rounded-box border-base-300 bg-base-100 hover:bg-base-200 border p-3 transition-colors motion-safe:transition-[opacity,transform] motion-safe:ease-in` — bordered, shadowless; avatar `size=sm` (default Avatar frame); preview bubble `bg-base-200 text-base-content/70 line-clamp-2 rounded-field px-2 py-1 text-sm`; timestamp `text-base-content/50 text-xs` |
| icons / hints | system icon `bg-warning/15 text-warning size-8 rounded-full` (aligns with `sm` avatar); type icons `text-base-content/60`; attachment hint `bg-base-300/40 size-10 rounded-field` + `text-base-content/70`; image thumb `border-base-300 size-10 rounded-field border object-cover`                                                                               |
...
| cards / badges / buttons | collapse cards `collapse-arrow rounded-box border-base-300 bg-base-100 collapse border` + content `border-base-300 border-t px-4 pt-4`; read-only `badge badge-sm badge-soft` + `badge-error`/`badge-success`; save `btn btn-primary` + loading spinner                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Import & export section  | `ImportExportSection.tsx`. Eyebrows `text-base-content/50 text-[10px] font-medium tracking-wide uppercase`. Export rows are one hairline-divided group — `rounded-field border-base-300 divide-base-300 divide-y overflow-hidden border` wrapping `min-h-11` row buttons (`hover:bg-base-200` + `focus-visible:ring-2 ring-primary ring-inset`, ring **inset** because the group clips); each row = `bg-base-300/40 rounded-field size-8` glyph disc (never `bg-base-200` — it vanishes on row hover) + `min-w-0 flex-1` label/extension stack + trailing 14px slot that swaps download → `loading loading-spinner loading-xs text-primary` → `Icons.check text-success`. Word and OpenDocument share `Icons.fileText`: the registry has no per-format glyph and `Icons.fileOpen` is an external-link arrow. Staged-import card + warning banner are `rounded-box` (cards): warnings `border-warning/30 bg-warning/10` + `role="status"`, capped at 5 items. Confirm is the only solid `variant="error"`. |
...
| sticky save footer | `bg-base-200 border-base-300 sticky bottom-0 border-t px-4 py-4` + primary btn `font-semibold` (with `h-16` spacer); footers (save + sign-out) add `max-md:pb-[max(1rem,env(safe-area-inset-bottom))]` for home-indicator clearance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| State | Recipe |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | -------------------------------------------------------------------------------------------------- |
| frame | Single `SettingsCard`: search row 1 + **toolbar row 2** + body; Documents tab `max-w-none w-full`; modal `size=5xl`. **Below `md` the pane is a full-bleed list screen, not an inset card** (list browsers go edge-to-edge; form panes keep the inset-grouped card): the tab body drops padding + goes `bg-base-100` (`fullWidth` fork in `SettingsPanel` + `max-md:` uncarding on `SettingsCard`), search+toolbar pin as a `sticky top-0` header on paper with a `border-b` hairline, rows/grid get `max-md:px-4`, and loading/error/empty states `max-md:flex-1` center in the remaining height — empty state adds a primary **Create document** CTA (`useNavigateToDocument`, promoted to `@hooks/`) |
| toolbar | Row 2 under search: **sort** `Select size=sm` named by a visually-hidden `<label htmlFor>` — **not** `aria-label` (closed `SelectProps`, excess-property error); wrapper `min-w-0 flex-1` + trigger `min-h-11` below `sm:`, `sm:w-40 sm:flex-none sm:shrink-0` + `sm:min-h-8` (label truncates — never fixed `w-40` on narrow). Right cluster: **Trash** icon-only `min-h-11 min-w-11` + `aria-label="Trash"` below `sm:` (label returns `sm:`); **view** `join` radiogroup `LuList` / `LuLayoutGrid`, each cell `min-h-11 min-w-11 sm:min-h-8`, active `btn-primary` / idle `btn-ghost border-base-300` |
...
| ⋮ menu | Shared `DocumentRowMenu` — **Popover** + `contextMenuPanelClassName` at `md+`; **below `md`** the same `RowMenuItems` render in an in-tree bottom action sheet (`RowMenuActionSheet`): `fixed inset-0 z-[60]`, `--modal-scrim` scrim (no blur), panel `rounded-t-box border border-b-0 border-base-300 bg-base-100` + grab handle + doc-name eyebrow + `min-h-12` rows + Cancel, `doc-region-in` enter. In-tree (never body-portaled): the settings modal's focus trap + outside-press dismiss must treat it as inside, and the full-screen blurred overlay is the fixed containing block; capture-phase window Escape closes the sheet before the modal's dismiss sees it (gallery-overflow recipe) — but bails while `globalDialog.isOpen` so a stacked confirm (Private ON, delete) gets the key first. The sheet card is `role="dialog"` + `tabIndex={-1}` and takes focus on open. List + grid reuse same component. Row order: Open in new tab / Copy link → **Rename** (`LuPencilLine`) → **Duplicate** (`LuCopy`) → `ContextMenuDivider` → Private / Read-only → `ContextMenuDivider` → **Delete** (`LuTrash2`, `ContextMenuRow variant="danger"`, **alone at the bottom**). Each action is a `<button type="button" class="rounded-field group w-full text-left">` wrapping `ContextMenuRow` (`group` is load-bearing for the danger `group-hover:bg-error/20`). Rename/Delete delegate up (`onRename`/`onDelete`); Delete + Rename `close()`, **Duplicate keeps the menu open** (mirrors the toggle patch) so its mutate-scoped `onSuccess` isn't dropped by an unmount. Duplicate: `toast.Loading` → additive `makeDocumentsKey` page-0 prepend (`total + 1` per page, no snapshot/rollback) → `toast.Success('Copy of “{title}” created', { id, actionLabel: 'Open', onAction })` — `onAction` is **required** or `ToastNotification` renders no button; **no auto-navigation**. Duplicate row `disabled:pointer-events-none disabled:opacity-60` while `isPending` (guards double-mint). |
...
| trash entry | Toolbar-row-2 affordance (**not** a nav tab): `Button size=sm variant=ghost startIcon={LuTrash2} text-base-content/60`, icon-only below `sm:` (`aria-label="Trash"`, `min-h-11 min-w-11`), labeled at `sm:`; grouped left of the view `join`. Click → `setShowTrash(true)`, which swaps the **whole card body** to `TrashSection` (search/toolbar/list hidden). Kept inside the Documents surface — no new `TabType`/`SETTINGS_TABS` plumbing. |
...
full-viewport shell — `components/pages/document/components/PrivateDocumentGate.tsx` (Approach B — brand-led)

| State            | Recipe                                                                                                                                                                                                                                                                                                                                        |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| frame            | Floor `bg-[var(--pad-well)]`; column `max-w-sm` + `doc-region-in`; brand above card (`DocsPlusIcon` + wordmark → `/`); card `modalPanelFrameClassName` + `px-6 py-8 sm:px-8` (no scrim); lock in `bg-base-200 size-12 rounded-full` disc (`/40`); title `text-lg font-semibold`; body `/70 text-sm`; slug caption below escapes `text-xs /50` |
| sign-in-required | Card primary only: **Sign in** `Button primary block min-h-12` → `openInlineSignInDialog({ returnTo })` + gate-owned `GlobalDialog`; under-card text: **Create a document** · **Homepage** (both → `/`)                                                                                                                                       |
| access-denied    | Card primary: **Create a document** → `/`; under-card **Homepage** only — **no Sign in**                                                                                                                                                                                                                                                      |
| cohesion         | Same lock disc language as DocumentsSection empty; no ghost/block secondaries inside the card; no Request access / landing hero / fake editor empty                                                                                                                                                                                           |
...
| read / edit modes | read: menu `btn btn-ghost btn-square min-h-11 min-w-11`, title `truncate text-lg font-semibold`; edit: Done `ToolbarButton text-primary min-h-11` (`onPress` → `releasePadEditMode`), undo/redo `ToolbarButton onPress min-h-11` + `divider divider-horizontal mx-2`; mode crossfade `motion-safe:animate-[doc-content-in_120ms_ease-out_both]`  |
| bell / avatar     | bell `text-primary` when unread else `text-base-content/70` + `UnreadBadge size=xs variant=error top-0.5 right-0.5`; avatar ghost circle wrapping `Avatar size=md` (default ring frame); signed-out `Button variant=neutral size=sm`                                                                                                             |
...
| State           | Recipe                                                                                                                                                                                                                                                                                                                                                                |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| frame           | wrapper `tiptap-toolbar-mobile motion-safe:animate-[doc-content-in_180ms_ease-out_both]`; main row `bg-base-100 z-10 h-14` + shared upward lift `$toolbar-lift-shadow` (`_toolbar.scss`); hidden unless editor && keyboard open; main-row `ToolbarButton` `min-h-11 min-w-11` + shared `.is-active` primary-10% recipe; comment uses `Icons.comment` (desktop parity) |
| format drawer   | `tiptap-toolbar-mobile__format-selection bg-base-100 absolute top-0 z-0 w-full -translate-y-full rounded-t-box px-4 py-6` + same lift shadow; visibility via `opacity` + `inert` (opacity-only motion — no JS height); format controls `min-h-11 min-w-11`                                                                                                            |
| heading stepper | `headingSelection rounded-field border-base-300 flex min-h-11 …`; label + `[-]`/`[+]` all `h-11 min-h-11 w-11` (44px); active `.is-active { background: color-mix(primary 10%); color: primary; border transparent; box-shadow: 0 0 0 1px color-mix(primary 44%) }` (`_mobile.scss`); disabled at level 1/6 or first-block Title                                      |

> Off-system: toolbar upward lift shadows (deliberate bottom-anchored species); ring-as-shadow on the heading stepper.

### Chat pane — mobile

docked — `components/pages/document/components/chat/ChatPane.tsx`, header + grabber in `components/chatroom/components/Chatroom/layouts/ChatroomPaneLayout.tsx`, geometry in `components/chatroom/utils/chatPaneGeometry.ts`

| State   | Recipe                                                                                                                                                                                                                                                                                     |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| frame   | flex child of `.mobileLayoutRoot`, last in the column: `chatPane bg-base-100 border-base-300 rounded-t-box flex shrink-0 flex-col overflow-hidden border-t`; inline `height` from `resolveChatPaneHeight`; `height` transition at `MOTION_PANEL_MS` ease-out, dropped under reduced motion |
| species | **L0 docked** — border, no scrim, no shadow. Not L2: the document behind stays live and scrollable, so there is nothing to dim                                                                                                                                                             |
| grabber | `grid h-11 w-full shrink-0 cursor-pointer place-items-center` + pill `bg-base-300 h-1 w-9 rounded-full`; a real `button` with `aria-expanded` and a focus ring — 44px, replacing the sheet's 20px non-interactive indicator                                                                |
| header  | `bg-base-100 border-base-300 flex w-full shrink-0 items-center gap-2 border-b px-3 pb-2`; breadcrumb `min-w-0 flex-1`, actions in a `bg-base-200 rounded-field` cluster (share, notification, close)                                                                                       |
| modes   | `closed` unmounts; `half` is the split ratio; `expanded` is the ratio at 1 clamped by the document floor, so ~96px of document always remains                                                                                                                                              |
...
| per-sheet config      | filters/link/mediaControls/mediaInsert `detent=content`; notifications/bookmarks/documentSettings default. Chat is not a sheet — see §Chat pane                                                                                                                                                                                                                                                                                                                                                                                                                      |
| panel sheet consumers | filters/bookmarks/notifications/documentSettings are 1-line wrappers passing `variant='sheet'` to the desktop panel (PanelSurfaceShell forks to SheetLayout); filter apply `SheetPrimaryFooter` "Apply filter" disabled when !query; sheet passes `onClose` for header Close; Reset shown in-sheet when chips present; dismiss via `useDismissPanel`                                                                                                                                                                                                                 |
...
| icon buttons          | history/filters/settings/bookmark: `Button ghost md square text-base-content/60 rounded-field hover:text-base-content hover:bg-base-300 focus-visible:ring-primary/30 focus-visible:ring-2 focus-visible:outline-none`; close is `CloseButton size=md` + `className="min-h-11 min-w-11"` (see Form & primitive foundations)                      |
...
| FAB elevation | `.edit-fab { box-shadow: var(--shadow-overlay) }` — L1 overlay token (\_mobile.scss); same language as popovers / media toolbar                                                                                                                                                                                                                                                                                                    |
...
> Off-system: tab/badge radii `calc(2rem/4)` / `0.35rem` literals.
...
| State   | Recipe                                                                                                                                                                                                                                                                                          |
| ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| frame   | `composer-bar__format-panel grid grid-cols-5 gap-1 px-2 py-2 motion-safe:animate-[doc-content-in_120ms_ease-out_both]` — no own fill (inherits mobile composer well); null unless mobile + toolbar open                                                                                         |
...
| State               | Recipe                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| frame               | shared prompt card `rounded-box px-5 py-4 bg-neutral text-neutral-content dark:bg-base-100 dark:text-base-content shadow-xl border-base-300 border`; PWA `fixed right-4 bottom-6 left-4 z-[60] max-w-md`, Notification `fixed top-6 left-6 z-[60] w-96`                                                                                                                                                                                                                                                                                                                                     |
...
| avatar / sign-in | ghost circle Button wrapping `Avatar size=lg` (default ring frame) + `transition-transform hover:scale-105`; signed-out `btn-soft btn-primary size-11 sm:size-12` circle; opens `ModalContent size=4xl/sm overflow-hidden p-0`                                                                                                                                                                      |
...
| controls                       | back/print/copy-link/compare = ToolbarButton (ghost sm square); compare uses `Icons.splitVertical` + `isActive` → `is-active` from `tiptap__toolbar` (no new CSS); restore `Button variant=primary size=sm font-normal` (only when !isLatestVersion, `loading`/`loadingText="Restoring…"` while in flight); version text `text-base-content/60 text-sm` with `text-base-content font-medium` date; right slot is the counter "Version N of M", replaced in compare by `A (time) → B (time)` + ghost xs Exit                                                          |
| block marks (read-only editor) | authorship `history-author-block` = `border-inline-start: 3px solid var(--color-primary)` + `color-mix(in oklch, var(--color-primary) 16%, transparent)` ground, `padding-inline-start: 9px` cancelled by `margin-inline-start: -12px` so marking moves no text; focused `history-author-block--current` = `outline: var(--focus-ring-soft)`. Diff `history-diff-added` = success 22% ground; `del.history-diff-removed` = error 18% ground + muted ink + line-through. Both partials live in `styles/editor/`, are reset in `_print.scss`, and never paint together |
...
| State             | Recipe                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| frame             | frame `sidebar bg-base-200 border-base-300 w-[25%] border-l`; sticky header `border-base-300 bg-base-200 sticky top-0 z-10 border-b px-3 py-3`; retention line under the count `text-base-content/60 text-xs`; desktop-only `PanelTabBar` (Versions · Authors) below the header, passed **no `count`** — its badge is `badge-error`, an unread affordance that reads as an error here                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| version rows      | idle li `group rounded-field min-h-10 transition-colors hover:bg-base-200/70`, dot `bg-base-300`, time `text-base-content/85`; active `bg-primary/10`, dot `bg-primary`, time `text-primary`; latest `badge badge-primary badge-sm`. Trailing group `ml-auto flex min-w-0 shrink items-center gap-1.5` carries the compare-base `A` marker, the trigger badge (`badge badge-ghost badge-xs`, never `badge-primary` — primary means selection here) and attribution (`Avatar size="xs" clickable={false}`, multi-writer → `AvatarStack surface="paper" maxDisplay={3}`, name `text-xs text-base-content/60`, active `text-primary/70`). A named row adds the message on a second line `text-base-content/70 mt-0.5 block truncate text-xs`; autosaves carry an empty message so density stays `min-h-10` |
| authors panel     | roster is a native `<fieldset>` of `radio radio-xs` rows (`min-h-9 rounded-field hover:bg-base-200/60`) so the browser supplies the W3C radio pattern; `None` checked on open; `Not recorded` is a static `text-base-content/60 text-xs` line with **no** radio; coverage line `text-base-content/60 text-xs` above the fieldset; Prev/Next `Button variant="ghost" size="xs"`; one `aria-live="polite" sr-only` region in the sidebar tree, never inside `.ProseMirror`                                                                                                                                                                                                                                                                                                                                |
...
control — `components/ui/CloseButton.tsx`

| State                  | Recipe                                                                                                                                                                                                                                                                                                                                        |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| frame                  | `Button variant=ghost shape=square` + `startIcon={Icons.close}`; `size=xs\|sm\|md` (default `sm`, 32×32 at `--radius-field` — the shape prop supplies the radius, never add one); touch-target callers pass `min-h-11 min-w-11` via `className`, never a new size variant                                                                     |
| idle / hover / active  | idle `text-base-content/70` + `hover:text-base-content` — the §State language icon-button recipe. Hover and pressed **fills come from daisyUI `btn-ghost`**; the component adds no fill of its own                                                                                                                                            |
| focus-visible          | `focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none`                                                                                                                                                                                                                                                                  |
| no per-ground override | `btn-ghost` hover resolves to an **opaque `base-300`** (measured: `oklab(0.890277 …)` = `rgb(220,227,237)`), so it is theme-tracked and identical on `bg-base-100` headers and inside the `bg-base-200` chat cluster. Do not re-add a `color-mix` fill for "ground independence" — the fill is absolute, not relative to what sits beneath it |
| exceptions             | `ChatMediaGallery/GalleryToolbar.tsx` "Close gallery" keeps its media-anchored white-on-black ink (`GalleryPillAction`, already `rounded-field`/square — a deliberate Color policy exception, not this component); `PWAInstallPrompt`/`NotificationPromptCard` dismiss stays hand-rolled — see that row                                       |
...
| State        | Recipe                                                                                                                                                                                                                                                                                                                              |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| frame        | `badge animate-badge-entry rounded-full leading-none font-semibold` + size (xs `text-[10px] min-w-4 h-4 px-1` … lg `text-sm min-w-7 h-7 px-2`) + variant `bg-{token} text-{token}-content` (all 8 daisyUI pairs); digits via `RollingNumber` (em-reel `translate3d`, not daisyUI `.countdown` — Safari clips that strip in h-4/h-5) |
