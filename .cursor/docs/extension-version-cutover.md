...
Rotating state for the five publishable `@docs.plus/extension-*` packages during Phase 1 (independent `2.0.0` → npm `@next`). Delete this file in the lockstep switch-flip PR that activates Phase 2. Authoritative policy: [RELEASE_POLICY.md](../../RELEASE_POLICY.md).

## npm publish status

| Package                                | Local `package.json` | On npm `@next` | On npm `@latest` | Git tag | GitHub Release |
| -------------------------------------- | -------------------- | -------------- | ---------------- | ------- | -------------- |
...
Update the table as each package ships. `@latest` stays empty until webapp-gated promotion (48h stable prod, no rollback).
...
5. **Publish:** `cd extensions/extension-<name> && bun publish --tag next --otp <6-digit>` — **Bun only**, not `npm publish` (`catalog:` peers)
6. **Tag:** `git tag '@docs.plus/extension-<name>@2.0.0'` then `git push origin '@docs.plus/extension-<name>@2.0.0'` — never `git push --tags`
7. **GitHub Release:** `gh release create '@docs.plus/extension-<name>@2.0.0' --notes "$(awk '/^## \[/{ if (found) exit; if (/^## \[2\.0\.0\]/) found=1 } found' extensions/extension-<name>/CHANGELOG.md)" --prerelease`
