...
| `lint`, `lint:fix`                         | ❌ Not per-package         | ESLint runs from the repo root in one process (`bun run lint` = `eslint . --max-warnings=0`); packages ship only the 3-line flat-config shim. See §Out of scope and AGENTS.md §ESLint Config.                  |
