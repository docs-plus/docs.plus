...
| `lint`, `lint:fix`                         | ❌ Not per-package         | ESLint runs from the repo root in one process (`bun run lint` = `eslint . --max-warnings=0`); packages ship only the 3-line flat-config shim. See §Out of scope (deliberate non-goals) and AGENTS.md §ESLint Config. |
...
- **Package metadata.** `name` is `@docs.plus/<kebab-case>`. `version` follows the contract in the `release-extensions` skill, §Extension Version Doctrine, for publishable libraries; private packages are free of lockstep.
...
- **A naming linter.** Prose + AGENTS.md + the `.cursor/rules/scripts-naming.mdc` card are the enforcement today. A `scripts/check-script-names.ts` stays out of scope; revisit only if regressions appear in PRs.
