...
/**
 * The single "editing is durably locked for this client" policy. It covers the
 * content-fork freeze (onContentError → providerStatus 'error'), a mirrored WS
 * `authorizedScope`, or the SSR read-only signal for a non-owner. Consumed two ways —
 * as a store selector (reactive, in useEditorEditableState), and via the imperative
