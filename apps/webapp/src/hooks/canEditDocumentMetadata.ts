...
export const canEditDocumentMetadata = (settings: Workspace, profileId?: string): boolean => {
  ...
  // fetch. A missing ownerId therefore reads the same as an open document while the
  // layout is still pre-sync. Answering true there flashes an editable title on a
  // document the caller cannot rename, so wait for documentId before deciding.
