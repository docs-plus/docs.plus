...
export const ChatroomBreadcrumbSkeleton = ({ className, variant = 'desktop' }: Props) => {
  if (variant === 'mobile') {
    return (
      <div className={twMerge('min-w-0 flex-1', className)} aria-hidden>
        ...
        <div className="skeleton mt-1 h-3 w-28 max-w-[min(100%,11rem)] rounded-md" />
      </div>
    )
  }
  ...
  return (
    <nav className={twMerge('flex min-w-0 flex-1 items-center gap-1', className)} aria-hidden>
      ...
      <div className="skeleton size-2.5 shrink-0 rounded-md" />
      <div className="skeleton h-3 w-20 max-w-[min(100%,8rem)] rounded-md" />
    </nav>
  )
}
