...
function DocumentPreviewPaper({ preview, title }: DocumentPreviewPaperProps) {
  ...
  return (
    <span
      ...
      {empty ? (
        ...
        <>
          {heading ? (
            <span className="mb-1.5 block text-[11px] leading-tight font-bold">{heading}</span>
          ) : null}
          {lines.map((line, i) => (
            <span key={i} className="text-base-content/80 mb-1.5 block text-[7.5px] leading-[1.35]">
              {line}
            </span>
          ))}
          {imageSrc ? (
