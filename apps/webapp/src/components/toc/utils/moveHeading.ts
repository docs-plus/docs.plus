...
export type MoveHeadingSectionArgs = {
  editor: Editor
  sourceId: string
  targetId: string
...
export function moveHeadingSection({
  ...
  position,
  newLevel
}: MoveHeadingSectionArgs): boolean {
  ...
  if (!source || !target) {
    console.warn('[moveHeadingSection] Could not find source or target', { sourceId, targetId })
    return false
  }
  ...
  if (target.sectionFrom >= source.sectionFrom && target.sectionTo <= source.sectionTo) {
    console.warn('[moveHeadingSection] Cannot move heading inside itself')
    return false
  }
  ...
  if (insertPos < 0 || insertPos > doc.content.size) {
    console.warn('[moveHeadingSection] Invalid insert position', {
      insertPos,
  ...
  try {
    ...
    console.error('[moveHeadingSection] Transaction failed:', error)
    return false
  }
