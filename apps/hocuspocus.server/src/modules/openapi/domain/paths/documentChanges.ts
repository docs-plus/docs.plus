    ...
    get: {
      ...
      parameters: [
        ...
        ...toParameters(changesQuerySchema, 'query', {
          since:
            'Required. Anchors the baseline, on `createdAt` and never on the version number. The newest row at or before this instant is used, so `baseline.createdAt` is usually earlier. Format `YYYY-MM-DDTHH:MM:SS`, an optional fraction, then `Z` or `±HH:MM`.',
          until:
            'Anchors the head the same way. Defaults to the moment the request is served. Equal to `since` is allowed and reports no change.',
          scope:
            '`summary` returns counts only. `headings` adds `sections`, the full outline tree including unchanged headings. `sections` is empty whenever no comparison ran.'
        })
      ],
      responses: {
        '200': {
