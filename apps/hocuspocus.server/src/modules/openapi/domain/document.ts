...
**Error envelope.** The canonical shape is \`ErrorEnvelope\`; \`error.details\` appears only when \`NODE_ENV=development\`. Several handlers still return ad-hoc shapes: \`LegacyError\` (email handlers, admin middleware, media-upload guards), \`LinkMetadataError\` (top-level \`code\`/\`message\`), and \`RateLimitError\` on 429. Wiring \`getErrorResponse\` everywhere is a separate task; the variants are documented as-is rather than smoothed over.
...
**Not described here:** \`POST /internal/documents/{documentId}/content\` lives on the collaboration process's internal listener (\`HOCUSPOCUS_INTERNAL_HTTP_PORT\`, default 4003), not on this server, and is not reachable at any \`servers\` URL.`
