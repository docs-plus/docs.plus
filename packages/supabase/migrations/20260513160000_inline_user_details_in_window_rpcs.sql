-- =====================================================================
-- 20260513160000_inline_user_details_in_window_rpcs.sql
-- =====================================================================
-- Fix: fetch_message_window / fetch_messages_since 403'd for anon and
-- authenticated callers with "permission denied for table users" because
-- they passed the whole `u` row composite to public.user_details_json(u).
-- Composite-row arguments require SELECT on every column of the table
-- (including `email`, which is REVOKE'd at column level from both anon
-- and authenticated). Inline `json_build_object(...)` only references
-- the five whitelisted columns directly, so security-invoker callers
-- succeed. Mirrors scripts/10-functions.sql.
-- =====================================================================
