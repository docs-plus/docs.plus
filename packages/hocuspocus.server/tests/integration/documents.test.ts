...
describe('Documents API', () => {
  ...
  describe('POST /api/documents/plugins/hypermultimedia/:documentId', () => {
    test('should upload file successfully to local storage', async () => {
      ...
      const response = await testServer.upload(
        '/api/documents/plugins/hypermultimedia/test-doc',
        file
      )
      const data = await response.json()
    ...
    test('should attempt S3 upload when configured', async () => {
      ...
      process.env.DO_STORAGE_KEY = 'test-key'
      process.env.DO_STORAGE_SECRET = 'test-secret'
      delete process.env.PERSIST_TO_LOCAL_STORAGE

      const file = createTestFile('test.png', 'image/png', 2048)
      const response = await testServer.upload(
        '/api/documents/plugins/hypermultimedia/test-doc',
        file
      )
      ...
      // S3 might succeed or fail depending on credentials, but should return proper response
      ...
      delete process.env.DO_STORAGE_KEY
      delete process.env.DO_STORAGE_SECRET
    })

    test('should reject upload without file', async () => {
      // Send empty FormData instead of JSON
      const formData = new FormData()
      const response = await testServer.app.request(
        'http://localhost:3001/api/documents/plugins/hypermultimedia/test-doc',
        {
          method: 'POST',
          body: formData
        }
    ...
    test('should return error when no storage configured', async () => {
      ...
      expect(data).toHaveProperty('error')
      expect(data.error).toBe('No storage configured')
    })

    test('should detect file type correctly', async () => {
      process.env.PERSIST_TO_LOCAL_STORAGE = 'true'

      const file = createTestFile('video.mp4', 'video/mp4', 2048)
      const response = await testServer.upload(
        '/api/documents/plugins/hypermultimedia/test-doc',
        file
      )
    ...
    test('should handle different file extensions', async () => {
      ...
      const file = createTestFile('document.pdf', 'application/pdf', 2048)
      const response = await testServer.upload(
        '/api/documents/plugins/hypermultimedia/test-doc',
        file
      )
  ...
  describe('GET /api/documents/plugins/hypermultimedia/:documentId/:mediaId', () => {
    test('should retrieve file from local storage', async () => {
      process.env.PERSIST_TO_LOCAL_STORAGE = 'true'

      const response = await testServer.get(
        '/api/documents/plugins/hypermultimedia/test-doc/test-file.jpg'
      )
    ...
    test('should retrieve file from S3 storage', async () => {
      process.env.DO_STORAGE_ENDPOINT = 'https://fra1.digitaloceanspaces.com'
      delete process.env.PERSIST_TO_LOCAL_STORAGE
