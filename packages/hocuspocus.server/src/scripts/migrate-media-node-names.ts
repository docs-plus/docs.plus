#!/usr/bin/env bun
// Batch rename of stored hypermultimedia media node types (PascalCase→camelCase)
// across full document history, pairing the 2.0 extension rename. Runbook —
// steps, data-loss posture, ENABLE_SCHEMA_MIGRATION lifecycle, memory ceiling:
// ../../docs/migrate-media-node-names.md. Flags: --dry-run (audit), --doc <id>.
...
function recordFailure(
  ...
  failures.push({ documentId, version, phase, message })
  const v = version !== undefined ? ` v${version}` : ''
  console.error(`  [${documentId}]${v} FAILED phase=${phase}: ${message}`)
}
...
async function migrateDocumentHistory(
  ...
  if (updates.length === 0) return { outcome: 'skipped', rows: 0 }
...
async function main() {
  ...
  console.log(`\n${'='.repeat(60)}`)
  console.log(`  Schema Migration: media node names PascalCase → camelCase`)
  ...
  try {
    ...
    console.log(`    Documents skipped:  ${stats.documentsSkipped} (no legacy media nodes)`)
