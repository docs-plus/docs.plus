...
export async function fetchUsers(
  ...
  let query = supabase
    .from('users')
    .select('id, username, email, full_name, avatar_url, avatar_updated_at, status, created_at, online_at', {
      count: 'exact',
    })
    .is('deleted_at', null)
...
export async function fetchNotificationStats(): Promise<NotificationStats> {
  const [totalResult, unreadResult, byTypeResult] = await Promise.all([
    ...
    supabase.from('notifications').select('id', { count: 'exact', head: true }).is('readed_at', null),
...
export async function fetchPushStats(): Promise<PushStats> {
  const [activeResult, failedResult] = await Promise.all([
    ...
    supabase.from('push_subscriptions').select('id', { count: 'exact', head: true }).gt('failed_count', 0),
  ]);
...
export async function fetchSupabaseStats(): Promise<SupabaseStats> {
  ...
  const [
    ...
    supabase.from('messages').select('id', { count: 'exact', head: true }).is('deleted_at', null).gte('created_at', todayStart.toISOString()),
    supabase.from('notifications').select('id', { count: 'exact', head: true }),
    supabase.from('notifications').select('id', { count: 'exact', head: true }).is('readed_at', null),
  ]);
...
export async function fetchTableSizes(): Promise<TableSize[]> {
  const tables = ['users', 'channels', 'messages', 'notifications', 'push_subscriptions', 'email_queue'];
