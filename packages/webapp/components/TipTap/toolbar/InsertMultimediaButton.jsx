...
const MediaRegex = {
  ...
  MusicFile: /^(?:(?:https?|ftp):\/\/(?:www\.)?[^\/]+\/|\/|\.\.?\/)?[\w\-\/\\]+.(mp3|wav|aac|flac|ogg|m4a|aiff|ape|asf|m4p|m4b|mp2|mpc|wma|webm|opus|ra|rm|wavpack|wv)$/i,
...
async function handleFileUpload(event, docMetadata, editor) {
  ...
  try{
   const response = await fetch(`http://localhost:2300/api/plugins/hypermultimedia/${docMetadata.documentId}`, requestOptions)
    ...
    const inputURLChange = (e) => {
      ...
      for (const [key, value] of Object.entries(MediaRegex)) {
        console.log({
          key,
          value,
          test: value.test(e.target.value)
        })
          ...
          <div className="flex items-center justify-center w-full mt-2 border-t pt-2">
              <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-36 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">Picture, Video or Audio (MAX. 2MB)</p>
                  </div>
                  <input id="dropzone-file" type="file" className="hidden" onChange={(e)=> handleFileUpload(e, docMetadata, editor)} />
              </label>
            </div>
  ...
  return (
    <Popover>
      ...
      <PopoverContent className="z-50">
        <HyperlinkForm variant="youtube" editor={editor} onSubmit={insertMedia} docMetadata={docMetadata} />
      </PopoverContent>
    </Popover>
  )
}
