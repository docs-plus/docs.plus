...
describe('E-2: findPrevBlock fallback for heading above scope', () => {
  ...
  test('H2 into [H3,H3,H3]: prevBlock should be FIRST block, not last', () => {
  ...
  test('H4 into [H5,H6,H7]: prevBlock should be first block (H4 < min level 5)', () => {
  ...
  test('H1 into [H3,H3] still uses last block (H1 is root, callers handle specially)', () => {
    const map: HeadingBlockInfo[] = [
      { le: 3, depth: 4, startBlockPos: 100, endBlockPos: 150 },
      { le: 3, depth: 4, startBlockPos: 150, endBlockPos: 200 }
    ]
...
describe('findPrevBlock fallback: same-level sibling detection', () => {
  test('H3 into [H3,H3,H3]: uses LAST same-level block as sibling', () => {
    const map: HeadingBlockInfo[] = [
      { le: 3, depth: 4, startBlockPos: 100, endBlockPos: 150 },
      { le: 3, depth: 4, startBlockPos: 150, endBlockPos: 200 },
      { le: 3, depth: 4, startBlockPos: 200, endBlockPos: 250 }
  ...
  test('H5 into [H3,H5,H3,H5]: uses last H3 as parent (findLast le < 5)', () => {
    const map: HeadingBlockInfo[] = [
      { le: 3, depth: 4, startBlockPos: 100, endBlockPos: 150 },
...
describe('getInsertionPos helper (DRY)', () => {
  test('returns endBlockPos - 2 when shouldNested is true', () => {
  ...
  test('returns endBlockPos when shouldNested is false', () => {
    const result: PrevBlockResult = {
  ...
  test('returns null when prevBlock is null', () => {
    const result: PrevBlockResult = {
      prevBlock: null,
      shouldNested: false
    }
...
describe('findPrevBlock backward compatibility', () => {
  ...
  test('normal case: H4 finds H3 as parent', () => {
  ...
  test('normal case: H2 finds H1 as parent', () => {
  ...
  test('single H1: H3 nests inside', () => {
