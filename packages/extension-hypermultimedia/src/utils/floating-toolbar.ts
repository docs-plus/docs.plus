...
export interface FloatingToolbarOptions {
  /** The reference element to position the toolbar relative to */
...
class FloatingToolbarManager {
  ...
  private createToolbarInstance(options: FloatingToolbarOptions): FloatingToolbarInstance {
    ...
    if (!referenceElement || !(referenceElement instanceof HTMLElement)) {
      throw new Error('Invalid reference element provided')
    }
    ...
    const show = async (): Promise<void> => {
      ...
      try {
        ...
        autoUpdateCleanup = autoUpdate(referenceElement, toolbar, updatePosition)
        instanceCleanup.add(() => {
        ...
        if (autoHide) {
          this.setupAutoHide(toolbar, referenceElement, hide, instanceCleanup)
        }
  ...
  private setupAutoHide(
    ...
    const handleOutsideInteraction = (target: Node | null) => {
      if (target && !toolbar.contains(target) && !referenceElement.contains(target)) {
