...
- **TOC drag grip + label rebuild.** Desktop row shell is `.toc__row` in `TocItemDesktop` — drag handle, title link, and chat trigger are **siblings** (never nest `<button>` inside `<a>`). Drag grip is an inline `shrink-0` flex item; do not restore `absolute -left-4` / `has-drag-handle`; row `overflow-hidden` plus TOC `ScrollArea` `overflow-x-hidden` clip outside-positioned handles. Grip is **hover-only** (opacity 0 + `pointer-events: none` until `.toc__item:hover > .toc__row`, same pattern as `.toc__chat-icon`; keyboard via `:focus-visible` only). Level picker UI is shared `TocLevelPicker` (H1–H6 only). Flat-schema `moveSection` moves the whole section (top-level H2 plus nested children) — drag E2E order assertions belong on `h2[data-toc-id]`, not `:is(h2, h3)`. TOC rebuild gate is `transactionRequiresTocRebuild` in `components/toc/utils/headingTransaction.ts` (meta fast paths + `ReplaceStep` position walk for in-heading text + `transactionAffectsNodeType` for structure).
