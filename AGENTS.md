...
- Long-form policy docs that an `.mdc` rule points at live in `.cursor/docs/`: `design-system.md` (source of truth for the webapp visual language — tokens, themes, elevation species, state recipes, component catalog), `scripts-naming-convention.md` (timeless rule, source of truth for scripts naming), `extension-version-cutover.md` (rotating per-package cutover state; see §Extension Version Doctrine), and `panel-feed-seams.md`. One-shot migration docs may live alongside as siblings and are deleted with the cutover PR that completes them.
...
- **`.superpowers/` is gitignored** (local agent scratch). Do not force-add or commit it; versioned plans the maintainer wants in git live under `docs/superpowers/` only.
...
- **X-embed CSP — resolved (2026-07-07).** The full X path is allowlisted end-to-end and verified against the emitted header: publish/syndication/platform hosts in `connect-src`, `platform.twitter.com`/`platform.x.com` in `script-src` (widgets.js is the only third-party script the editor injects) and `frame-src` (plus syndication); widget-internal media loads under X's own CSP, parent-context images ride the `img-src https:` wildcard. Hosts live in the `X_*` groups of `apps/webapp/config/security/third-party-hosts.js`, spread into `next.config.js` — X embeds do not degrade to the seeded blockquote in production, and the blockquote fallback needs no grants. Future X endpoint changes are a one-line `X_*_HOSTS` edit plus a server restart.
...
- **Foreign attribute mutations inside the editor recreate node views (media embeds reload).** ProseMirror's `DOMObserver` watches every attribute on every descendant (no `attributeFilter`) and reconciles non-PM mutations by re-rendering the dirty range — destroying+recreating the affected node views (`NODE_DIRTY` set by `docView.markDirty` over the unioned range; `CustomNodeViewDesc.update` returns `false` before consulting `spec.update`, so a node-view's own `ignoreMutation`/`update` can NOT veto a range-set dirty — sibling mutations recreate it as collateral). Consequences and wiring:
  ...
  - Related latent footgun: `syncHeadingWidgetUnread` writes `dataset`/`style` to `.ha-chat-btn` every unread tick — safe ONLY because those are `Decoration.widget` DOM (DOMObserver ignores widget attribute mutations); keep them widget DOM, never a schema node / NodeView content child.
...
- **Canonical CSS tokens** in `apps/webapp/src/styles/globals.scss` (`:root`; dark overrides on `[data-theme='docsplus-dark']` / `docsplus-dark-hc`):
  ...
  - `--pad-sheet-radius: var(--radius-box)` — chains to the daisyUI shape tokens, which are the app-wide radius control panel (`--radius-selector` 8px / `--radius-field` 8px / `--radius-box` 10px in every theme block; maintainer-tuned taste is 8–10px, no more, no less). Surfaces consume `rounded-selector|field|box` (or `var(--radius-*)` in SCSS) — never Tailwind literal radius classes.
...
- **Media-anchored overlay ink is fixed black/white by design** (upload-progress scrims on thumbnails, avatar-hover camera overlay, video/gallery letterbox): those sit on imagery, where black scrim + white ink holds in both themes — do not "tokenize" them to `neutral`/`base-content`, which turns into a gray wash in dark mode.
...
- **TOC resize** (`useTocResize.tsx`, `ResizeHandle.tsx`, `SlugPageLoader.tsx`): max width `TOC_MAX_WIDTH_RATIO = 0.46` of `.editor` row; clamp on drag + window resize. Vertical sash mounts on the `.editor` row (not inside the TOC column) at `left: tocWidth`, `z-[41]`, straddling the split (`right-[calc(var(--resize-sash-hit)/-2)]` on a zero-width anchor) so it stays the **sole** continuous hairline above the docked chat (`z-40`) and **below** the TOC column (`z-[42]`) — presence overhang paints over the hairline; Dialog/Popover (`z-50`) stay above both. Do not nest the sash under TOC `isolate` or chat paints over it and the panel edge reads as a thicker, offset second line. Do not bump TOC to `z-[51]+` (that stacks a docked rail above modals). Always draws a 1px idle hairline (`--resize-sash-idle`); widens/recolors to primary on hover/drag. Do not reintroduce column `border-r` or idle `after:opacity-0`. Horizontal chat sash: **`bottom-full`** on the panel top edge — never `top-0 -translate-y-1/2` (overlaps chat toolbar and steals clicks).
...
- **Unified floating panel frame** — popovers, context menus, and modal cards share one elevation language:
  - `popoverPanelClassName` (`Popover.tsx`), `contextMenuPanelClassName` (`ContextMenu.tsx`), `modalPanelFrameClassName` / `modalPanelClassName` (`Dialog.tsx`): **`rounded-box`** (the daisyUI shape token — 10px today, retunable from the theme blocks alone), **1px `border-base-300`**, **`shadow-xl`**, **`bg-base-100`**. Never a Tailwind literal radius class (`rounded-lg/xl/2xl`) and no per-feature `PANEL_CLASS` string modules. The full popover export is `rounded-box border-base-300 bg-base-100 z-50 w-[28rem] overflow-hidden border p-0 shadow-xl`; every desktop toolbar/pad `PopoverContent` (bookmarks, document-settings, filter, `PadTitle`) wraps in it — deliberately the sibling of `contextMenuPanelClassName`, so popovers and context menus speak one floating-surface language in light and dark. SCSS floating panels use `border-radius: var(--radius-box)` + `var(--shadow-overlay)`.
...
- **Offline mirror.** `useYdocAndProvider` pairs each Y.Doc with an `IndexeddbPersistence` keyed by the provider room name (`documentId`), created and destroyed inside the ydoc-owning effect (persistence teardown first, before the provider) — never reused across doc switches. It replays local bytes before first `onSynced`; the `providerSyncing` gates above keep ymetadata consumers post-merge. The merge alone is NOT safe for never-persisted drafts: isDraft docs skip store(), so every reopen meets a fresh server default state whose `needsInitialization=true` can win the Y.Map merge coin-flip against the mirror's `false` — `useInitializeNewDocument` therefore also content-gates the seed on the fragment's **text** (never `setContent` over user text). Node count is the wrong predicate — the editor binding writes its empty mandatory-heading scaffold into the `default` fragment before the seed effect runs, so even a genuinely fresh doc is non-empty-but-textless at seed time (a node-count gate blocks every first seed; browser-verified 2026-07-15). Residual accepted: a media-only never-persisted draft has no text and could still be re-seeded. Cached doc bytes stay readable by later users of the browser profile — accepted, not solved.
...
- **Private is owner-only end-to-end.** Shared `resolvePrivateAccess` (REST slug read + WS via `resolveWsAccess`, fail-closed on metadata lookup failure); anonymous → `sign-in-required`, signed-in non-owner → `denied`; webapp `PrivateDocumentGate` (`sign-in-required` | `access-denied` via `toPrivateGateVariant`) mounts before the collab provider so blocked viewers never open a WS room.
- **Private and Read-only must not both be ON.** Under owner-only Private, Read-only is meaningless to non-owners (they cannot open the doc). Turning Private ON clears Read-only in the same PATCH and disables the Read-only control until the doc is public again — same rule on `DocumentSettingsPanel` and Settings Documents `DocumentRowMenu` (⋮).
...
- **Access mutation path.** Owner (or ownerless first-claimer) changes flags via `canMutateAccessFlags` + `useDocumentAccessMutation` (Settings panel + Documents ⋮ share the hook). Live seal: REST publish → Redis `doc:{id}:access` (`accessRealtime`) → WS broadcast/close → client `applyAccessStateless`.
...
- **Canonical row (mobile + desktop): `[ + ] field [ 😊 ] [ 🎤/✈ ]`** — no `@` or separate Aa/attach buttons in the row (type `@`; mention is also in the format toolbar). The left **`ComposerInsertMenu`** opens on click/tap (no tooltip, no hover-open); rows = Attach file, Text formatting (toggles the format bar), Record voice when send is showing. **`ComposerPrimaryAction`** switches mic ↔ ghost **`text-primary`** send when `canSend` (same control species as + / emoji / mic — not a filled primary disc), and renders `null` during voice preview (the bar owns attach). **`VoiceNoteButton`** / **`ComposerOverflowMenu`** stay deleted.
...
- **Gallery overflow menu.** Desktop: inline dropdown anchored under the ⋯ pill (`bottom-end`, not portaled `ui/Popover` — outside-click dismiss steals/clobbers portaled menus). Mobile: bottom action sheet **portaled to `document.body`** (`fixed inset-0` scrim + `rounded-t-box` sheet, gallery theme vars reapplied on the portal root, `min-h-12` rows, icon-leading, inline View Details, Cancel) — never nest `position: fixed` under the pill’s `backdrop-blur` (that containing block collapses the sheet into the ⋯ button). Escape closes the menu before the gallery. Same action list from `galleryToolbarModel.ts`.
- **Gallery copy actions.** Forward / overflow “Copy Media Link” must copy the active slide’s already-resolved `http(s)` URL synchronously on click (`readGalleryActiveMediaUrl()` after `GallerySlide` publishes via `publishGalleryActiveMediaUrl`) with success/error toasts — async sign-after-click loses the clipboard user gesture and fails silently without feedback.
...
- **Draft identity is anchored on first edit.** A never-persisted draft's `documentId` (minted per-request by `createDraftDocument` in `documents.service.ts` — still random `uid.stamp(19)`, NOT a slug hash) is BOTH the client `IndexeddbPersistence` key AND the Hocuspocus room name, so before the first persist a reload used to mint a NEW id, rotate both keys, and orphan the early edits (blank editor — the "content-disappear" class). `firstEditMetadataExtension.onChange` (`hocuspocus.server.ts`) closes it: on the first `isDraft` → `false` flip it eagerly writes the slug→`documentId` `DocumentMetadata` row via `ensureDraftDocumentMetadata` (idempotent — `P2002` = already anchored, or a concurrent first-open won the still-`@unique` slug and we cede), WeakSet-guarded once per loaded Document (keyed by the Document object → auto-GC on unload, no per-keystroke writes). It anchors IDENTITY only; content/versions still persist via the debounced `store()`. **The worker's `upsertDocumentMetadata` must never OWN metadata — its UPDATE branch is `update: {}` (no-op); CREATE is the sole no-row backstop.** A content persist re-asserting `title=slug`/`ownerId` on the first version reverted a user-set title to the slug on reload and could flip ownership to the last flusher (the anchor / `updateDocument` PUT / `createDocument` are all more authoritative). So a reload's `getDocumentBySlug` returns the same id → same IndexedDB + room → the mirror restores the edits. Full-stack regression coverage: `apps/webapp/cypress/e2e/document/draft-first-edit-anchor.cy.ts` (survives pre-debounce reload · no row without an edit · renamed title survives first persist) — real-stack, DB-asserted via `docker exec` `psql` cy.tasks against `docsy-postgres-local`, needs `make dev-local`. Bots that open and never edit keep `isDraft` and stay row-less (anti-empty-doc). Keep the id RANDOM: access is authz-gated (`resolveWsAccess`), so unguessability is defense-in-depth, not the access control, and a slug-derived id would be guessable for no authz gain. Residual: a sub-second window before the anchor's `onChange` → DB write lands (far smaller than the old debounce-length window; a slug-deterministic id would zero it at the cost of guessability).
- **Document state rides a claim-check Redis key** (`store-doc-state:<jobId>`, 1h TTL, binary — no base64); BullMQ job data carries only `stateKey`. The worker still accepts legacy inline `state` (in-flight jobs across deploys), and the DLQ handoff embeds the base64 payload so recovery survives the key TTL — but only when the worker held the state at failure time. A job that sits in `wait` past the TTL before its FIRST attempt (worker outage or deep backlog) would dead-letter payload-less; the store()-side direct-DB fallback does not cover this either (it fires only when enqueue throws, not when the worker is down). Closed after the 2026-07-14 outage by `refreshPendingStateKeyTtls` (`lib/queue.ts`): the WS server (producer side, where enqueue lives) re-arms pending jobs' state-key TTLs every 10 min, covering `waiting`/`paused`/`delayed`/`active`/`failed`; keys stay TTL'd so volatile-lru under OOM remains the accepted backstop loss path.
...
- **Autosave retention never deletes named history.** The worker's hourly cleanup thins unnamed autosave rows older than `DOC_AUTOSAVE_RETENTION_DAYS` (default 30, `0` disables) to one per document per day; rows with a `commitMessage` are user-visible checkpoints served by the history sidebar and must never be auto-deleted.
...
- Both version-append paths (the worker INSERT and the queue-down inline fallback in `store()`) merge the snapshot with the locked head row (`Y.mergeUpdates`) so an out-of-order commit or a divergent concurrent fallback can never clobber newer edits into the latest version. Keep them in lockstep — do not let one merge and the other plain-insert.
...
- Prod deploy order: `hocuspocus-worker` (scale 2, matching compose replicas) before `hocuspocus-server` — consumers learn a new job shape before producers emit it.
...
- Chat media gallery zoom/pan/keyboard and mobile ⋯ overflow changes are not done until verified in the browser (desktop click/wheel/keyboard; iOS pinch/double-tap/pan; mobile overflow opens a body-portaled bottom sheet — never nested under the pill’s `backdrop-blur`); do not claim fixed from code inspection or DevTools assumptions alone.
- Hypermultimedia media-toolbar ⋯ overflow must toggle on repeated click and sit flush under the toolbar’s right edge (`bottom-end` on the bar, tight top gap) — not a large offset under the ⋯ alone; `@docs.plus/floating-popover` outside-dismiss must ignore the toggle trigger or the menu race-reopens and never appears to toggle.
- Chatroom feed error / empty-load states should stay Discord-like and minimal: calm plain-language copy (no raw `403` / `permission denied` strings), design-system tokens, and the same composition on mobile sheet + desktop docked panel — research/compare before shipping a louder custom layout.
...
- Email notification path: pg_cron SQL → `pgmq.send('email_notifications_queue')` → hocuspocus-worker `consume_email_queue` RPC → BullMQ → SMTP/Resend/SendGrid (pg_cron never calls HTTP).
- Prod `public.email_queue` check constraint (`email_queue_status_check`) can drift from repo and omit `processing`, causing `compile_digest_emails` to fail until aligned with `scripts/07-5-email-notifications-pgmq.sql`.
...
- Preflight env bootstrap: on first run it creates `.env.development` from `.env.example` and writes `.env.local` through `localizeEnvForHost` (REDIS_HOST/SERVER_RESTAPI_URL/SUPABASE_URL → localhost, DATABASE_URL appended from DB_USER/DB_PASSWORD/DB_PORT). It never mutates an existing `.env.local` — it only notes key names missing vs `.env.example`. Keep the Docker→localhost mapping in `LOCAL_HOST_OVERRIDES`, nowhere else.
...
- `concurrently` is a pinned root devDependency consumed via `bunx` in `dev-local`/`dev-backend` with `--kill-others-on-fail` (one crashed process tears the run down loudly). Do not remove the pin — unpinned `bunx concurrently` fetches latest from npm at dev start.
- `SUPABASE_SERVICE_ROLE_KEY` in `.env.example` is the Supabase CLI's public local-demo JWT (same demo-secret family as the anon key) so the admin gateway and worker queue consumers work on a zero-edit clone; cloud/prod replaces it.
...
- `@docs.plus/document-swarm` (`packages/document-swarm`) is a private, never-published internal Bun + Playwright CLI that drives N seeded browser "Swarm Actors" against a public editable document URL in Demo (paced, headed) or Stress (higher-concurrency, headless) mode — glossary in `CONTEXT.md` §Document swarm; subcommands are `provision` (idempotently seed Supabase auth users via the Admin API into `actors.json`) and `run --mode demo|stress --users N --url <doc>`; Make sugar is `swarm-demo` / `swarm-stress`. Stable invariants: the host allowlist refuses production (local + `stage.docs.plus`/`*.stage.docs.plus` only), per-host Actor caps (local 20 / stage 10) need `--force` to exceed, the Swarm Target must be public and not Read-only/soft-deleted, actors sign in by injecting the `@supabase/ssr` chunked `sb-<ref>-auth-token` cookie via `context.addCookies` before navigation (not localStorage, not Google One Tap), pad writes are non-destructive (never select-all / wipe the doc — each actor resolves a Write Target / section; Demo contention defaults section-isolated; optional Shuffle uses a weighted script mix), first Ctrl+C must immediately `browser.close()` so in-flight Playwright waits abort (do not hang draining actor loops; second SIGINT or ~5s timeout → `process.exit(130)`), and its `eslint.config.js` uses the base `eslint-config/index.js` preset (not `library.js`) because console is the CLI interface.
