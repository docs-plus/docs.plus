...
- **Feed visual layout (ratio-first hybrid).** Image+video share one `MessageMediaVisualBlock` sized from intrinsic dims into max width×height (`computeVisualMediaLayout`): `single` clamp; hybrid `mosaic` vs `stack` (stack only when every resolved aspect is tall **and** count ≤ `CHAT_MEDIA_VISIBLE_CAP` — overflow always mosaic so `+N` has a home); unknown dims use 16:9 placeholders (never stack). Audio/file stay as cards below. Mosaic/stack video is `MessageMediaVideoPoster` → lightbox; lone video is inline `MessageMediaVideo` + Expand. Lightbox playlist stays `[...images, ...videos, ...audio]` via one `partitionMessageMedias` + `openGallery(medias, at?)` — open by media object, never feed cell index. Signed-URL failures surface `MediaUnavailable` + retry. Reply/edit bars use `ContextBarMediaThumb`.
...
- Settings Documents list UX should follow a Google Docs-like per-row ⋮ menu (open / copy link when public / private / read-only), not inline visibility toggles — keep it minimal and size-fit for the settings panel; Private ON must clear and disable Read-only on that menu and on `DocumentSettingsPanel` (see §Document Access).
...
- Chat feed image tiles must be Discord-style ratio-first, not locked to a fixed crop box: a single image scales down from its intrinsic width/height while preserving aspect ratio; multi-image is hybrid — mosaic/`object-cover` when ratios fit the group, stacked natural-ratio tiles when tall screenshots would lose too much to cover-crop.
...
- Chat media gallery zoom/pan/keyboard changes are not done until verified in the browser (desktop click/wheel/keyboard and iOS pinch/double-tap/pan); do not claim fixed from code inspection or DevTools assumptions alone.
