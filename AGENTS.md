...
- **Canonical CSS tokens** in `apps/webapp/src/styles/globals.scss` (`:root`; dark overrides on `[data-theme='docsplus-dark']` / `docsplus-dark-hc`):
  ...
  - `--shadow-overlay` — the one box-shadow for every SCSS-styled floating panel (dropdown-menu, media-toolbar, hyperlink popovers, emoji picker, Dialog SCSS, TOC drag levels). Black-based with a stronger dark override — never `base-content` mixes, which glow in dark. Tailwind-styled floating surfaces use `shadow-xl`. No other floating shadow literals outside the deliberate exceptions listed in `design-system.md` §Elevation (drag card/deck, Edit FAB, crinkle folds, bottom-anchored mobile bars, tooltip tier).
...
- **Document state rides a claim-check Redis key** (`store-doc-state:<jobId>`, 1h TTL, binary — no base64); BullMQ job data carries only `stateKey`. The worker still accepts legacy inline `state` (in-flight jobs across deploys), and the DLQ handoff embeds the base64 payload so recovery survives the key TTL.
...
- Settings Documents list UX should follow a Google Docs-like per-row ⋮ menu (open / copy link when public / private / read-only), not inline visibility toggles — keep it minimal and size-fit for the settings panel.
...
- Private documents are owner-only end-to-end: shared `resolvePrivateAccess` (REST slug read + WS via `resolveWsAccess`, fail-closed on metadata lookup failure); anonymous → `sign-in-required`, signed-in non-owner → `denied`; webapp `PrivateDocumentGate` (`sign-in-required` | `access-denied`) mounts before the collab provider so blocked viewers never open a WS room.
