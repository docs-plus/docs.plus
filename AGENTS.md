...
- TipTap extension SCSS must be imported through the styles chain (`styles.scss` → `components/_index.scss`); missing imports cause decorations to apply but remain invisible (no compile error)
