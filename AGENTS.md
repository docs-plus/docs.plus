...
- **Gallery overflow menu.** Inline dropdown anchored in the toolbar — not portaled `ui/Popover` inside the modal (outside-click dismiss steals/clobbers portaled menus; controlled `Popover` also disables Floating UI `useClick` — manual trigger toggle like `ComposerInsertMenu` if Popover is unavoidable elsewhere).
