...
- **ChatMediaGallery UX (Discord parity).** Full-bleed lightbox (`modalBackdropHeavyClassName`, `z-[100]`, `h-[100dvh]`, `overscroll-none`): top-left author block (name line 1, sent-at line 2); top-right grouped action pill (zoom on image slides only, reply when `originMessage`, download, open, ⋯) plus separate close; copy link lives in overflow only; `n / total` counter when multi-item. Tap letterbox/stage (not controls) toggles the overlay controls when not zoomed (`controlsVisible`, `MOTION_PANEL_MS` fade). Spoiler feed tiles (image, video poster/lone video, audio) stay **two-tap reveal** before `openGallery` — carousel must not bypass feed spoiler state.
...
- **Gallery overflow menu.** Inline dropdown anchored in the toolbar — not portaled `ui/Popover` inside the modal (outside-click dismiss steals/clobbers portaled menus; controlled `Popover` also disables Floating UI `useClick` — use a manual trigger toggle if a controlled Popover is unavoidable elsewhere).
- **Gallery copy actions.** Forward / overflow “Copy Media Link” must copy the active slide’s already-resolved `http(s)` URL synchronously on click (`displayUrl` from `GallerySlide` → `copyResolvedMediaLink`) with success/error toasts — async sign-after-click loses the clipboard user gesture and fails silently without feedback.
...
- **Gallery spoilers.** Image **and** A/V slides use `useFeedSpoilerGate` from `feedSpoilerReveal` (same module as the revealed Set); unrevealed A/V show the reveal chip and never autoplay/controls until revealed — carousel must not bypass feed two-tap.
- **Gallery slide controllers.** Active slides register zoom/media handles via `registerGallery*Controller` in `chatMediaGalleryStore.ts` (keep register in that file — Next HMR can duplicate a separate utils singleton). Pure zoom math lives in `galleryZoomMath.ts`; gesture DOM stays in `useGalleryZoomPan`.
