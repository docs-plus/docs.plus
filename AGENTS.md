...
- **`@docs.plus/floating-popover` is bundled into each consuming extension's `dist`, never externalized.** It is a `private` workspace package (never published), wired as a `devDependencies` `workspace:*` in `extension-hyperlink` and `extension-hypermultimedia`. Their `tsup.config.ts` `external` must stay `['@tiptap/core', '@tiptap/pm']` — do **not** add `'@docs.plus/floating-popover'`, or tsup emits a bare `import … from '@docs.plus/floating-popover'` into the published bundle that throws `MODULE_NOT_FOUND` for external npm consumers. `@floating-ui/dom` stays external (real published dep). Verify after any tsup change: built `dist/index.{js,cjs}` must contain no `@docs.plus/floating-popover` import/require.
...
- **Build `@docs.plus/floating-popover` before the extensions that bundle it — inside Docker too.** hyperlink and hypermultimedia bundle it (not external; see §Shared Library Config), so any stage that runs their `bun run build` must first `COPY packages/floating-popover` and `RUN cd packages/floating-popover && bun run build`. Wired in webapp `build-extensions` and hocuspocus.server `base`. admin-dashboard builds no extensions but still copies `packages/floating-popover/package.json`, or the extensions' `workspace:*` reference fails to resolve under `--frozen-lockfile`.
...
- Cypress lives under `apps/webapp/cypress/e2e/editor/indent/`; Jest lives under `extensions/extension-indent`.
...
- After substantive chatroom/webapp or Supabase cleanup, expect a cohesive DRY/KISS pass (dead-file import-graph sweep, redundant store fields, code-reviewer/code-simplifier) before treating the work as production-ready—not optional follow-up polish.
...
- Monorepo dependency bumps go through `npm-check-updates` (`bun run update` = patch+minor; `bun run update --upgrade` for majors)—not `npm-check`, not a verbose `bun update --recursive` wrapper; keep `scripts/update-packages.sh` minimal.
...
- **`extension-hypermultimedia` embed configuration.** `@docs.plus/floating-popover` (extracted from hyperlink; local popover impl deleted there) positions media toolbars. **X** uses oEmbed width presets (Compact 280 / Standard 400 / Wide 550) and light/dark theme via toolbar—not gripper resize; exclude `x` from gripper/`acceptedNodes`. `hide_media`, `hide_thread`, `lang` (+ kit `dnt`) are kit defaults and per-node attrs for integrators, not toolbar controls. **YouTube** maps official iframe query params in `nodes/youtube/embedOptions.ts` (kit defaults + per-node attrs); paste extracts `start` from `t=?`/`start=`; `loop=1` auto-sets `playlist` to the video id; no toolbar UI for player params unless explicitly requested. **Iframe embed nodes** (YouTube, Vimeo, SoundCloud, Loom) share `createIframeEmbedNodeView` / `createIframeEmbedRenderHTML` via `src/utils/iframeEmbedNode.ts`; per-host URL/attr logic stays in each `embedOptions.ts`. Shared kit in `src/utils/embedKit.ts`: `resolveEmbedOption`, `embedAttrsEqual`, `resolveEmbedLayoutDimensions` (insert-time column clamp for default 640×480 embeds). **Loading shell:** `loadingShell` kit option (default `true`; factory override); `src/loading/` + `media-loading-shell.css` (`data-hm-loading`, `--hm-loading-*`); clipboard file upload progress stays webapp (`mediaUploadPlaceholder`); X uses fluid `hm-media-host--fluid` after widgets.js settles.
...
- **`extension-hypermultimedia` has a clean-room Cypress E2E harness.** Specs live under `extensions/extension-hypermultimedia/cypress/e2e/**` (kebab-case dirs) and run via `bun run test`, which uses `start-server-and-test` against the Bun playground at `http://127.0.0.1:5174` (`pretest` builds `dist/` first). Shared helpers `activateImageGripper` / `hoverMediaControls` / `dragResizeClamp` / `expectMediaLoadingPending|Ready` live in `cypress/support/e2e.ts`; `cypress/e2e/loading/media-loading-shell.cy.ts` covers the loading overlay. Resize specs assert rendered inline size on `img`/iframe/video/`.hm-media-host`, not attrs alone; caption specs assert node survival via `nodeCount` and use caret-based backspaces — not `{selectall}` (which escapes the nested figcaption editing host and selects the whole doc) or `{moveToStart}` (which moves focus out of the caption). Start the playground hot-server as a managed background process, not ad-hoc `nohup` from a tool shell (it is killed when the shell returns). A leftover playground process leaves port 5174 `EADDRINUSE` between runs — clear it with `lsof -ti:5174 | xargs kill -9`.
