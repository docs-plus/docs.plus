...
- **Production observability + landing shell.** Observability stack in `docker-compose.observability.yml` (Grafana/GlitchTip/Telegram; add `GLITCHTIP_DSN` + `NEXT_PUBLIC_GLITCHTIP_DSN` to prod `.env` after bootstrap). Deploy commit subject `(build): <domains>` — canonical order `back → front → observability → uptime-kuma` (`.github/WORKFLOWS.md`). Landing `/` is SSG with deferred anon auth via `routePolicy.ts`; `document-styles.scss` dynamic-import only when `documentShell`; mobile home keyboard uses `useHomeKeyboardCompact` + visual viewport sync on slug focus.
