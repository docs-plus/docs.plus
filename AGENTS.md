...
- Chat feed image tiles are Telegram-album style: single scales from intrinsic size; multi-image uses absolute rects (Layouter 2–4 + ComplexLayouter) with cover-crop inside cells — not a fixed Discord grid and not a separate tall-stack mode.
