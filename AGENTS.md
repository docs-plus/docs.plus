...
- **TOC chat trigger is `<button type="button">`** (was `<span onClick>` inside `<a>`) for keyboard + a11y semantics; same pattern as mobile. The desktop row shell is `.toc__row` in `TocItemDesktop` — drag handle, title link, and chat trigger are **siblings** (never nest `<button>` inside `<a>`). The chat hover SCSS selector is `&:hover > .toc__row .toc__chat-icon`, NOT the deeper `&:hover > a > span > .toc__chat-icon` — the rail DOM broke the original deeper selector. `usePresentUsers` (`@hooks/usePresentUsers`) MUST filter `profile?.id` (matches `PresentUsers.tsx`) and keep per-channel `useShallow` equality so presence Map replaces do not re-render every TOC row. TOC focus/chat-active styling MUST use boolean Zustand selectors (`focusedHeadingId === item.id`, `chatRoom.headingId === item.id`) — selecting the raw id string re-renders all N rows on every scroll-spy tick. `useTocAutoScroll` must subscribe outside the outline React tree (store.subscribe), never via a parent `useStore` that would remount the list.
...
- TOC tree connectors use daisyUI’s built-in nested menu guide (`.menu :where(li ul)::before` on `toc__children`); vertical rails only — no L-elbows / custom per-row connector components; strengthen with `base-content` ~24% mix (raw `--pad-divider` on `--pad-well` is nearly invisible).
- TOC chat buttons and unread badges stay on one vertical x-position across `TocHeader` and every row (existing `TocRowTrail` contract); do not let header/row trail drift independently.
