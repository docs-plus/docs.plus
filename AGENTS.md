...
- Retry path: failed-row tap → `ChatroomContext.retry(clientId)` → `useSendMessage.retry` → `components/chatroom/utils/retryMessage.ts` helper (the only remaining caller of the `sendMessage` API). Re-issues with the original client UUID; 23505 duplicate-key is treated as success (the row was already persisted on a prior attempt). Returns `RetryMessageResult`; the caller (`useSendMessage.retry`) owns the pending/sent/failed UI flip against Virtuoso's data.
...
- **History loading bones.** `HistoryEditorContent.tsx` (`variant: 'desktop' | 'mobile'`) must use shared `EditorContentSkeleton` with matching `SKELETON_CLASS` / `EDITOR_CLASS` records (`mb-12 border-t-0 px-6 pt-8 sm:mb-0 sm:p-8` desktop; `docy_editor relative w-full` mobile) — not inline `DocumentSimpleLoader` in a bare `ProseMirror` wrapper. Read-only history TipTap lives in `useStore((s) => s.editor)`; pad collab uses `settings.editor.instance` (`PadEditorLifecycle` unmounts on `#history`). In-doc `#history` navigation must use `pushAppUrlThenNotifyHashChange()` in `historyShareUrl.ts` — shallow `Router.push` updates the URL but does not fire `hashchange`, so `useHashRouter` never mounts the history layout. Sidebar lists virtualize with Virtuoso at 80+ rows (`HistorySidebarBody`).
...
- **`extension-hypermultimedia` media-resize chrome is hover-driven on desktop (Notion-style).** `MediaResizeControls` (`src/extensions/mediaResizeControls.ts`) wires `mouseover` / `mouseout` / `click` through `handleDOMEvents`; per-editor state lives in `WeakMap<Editor, MediaControlsState>` at `src/utils/media-resize-controls.ts` (not module globals). Desktop is gated by `hasFinePointer()`; `ResizeGripper` uses `acceptedNodes` (resizable only), `MediaResizeControls` uses `trackedNodes` (all media incl. X for delete teardown). Interactive embeds (`isInteractiveEmbed` — everything except `image`/`audio`) stay clickable: gripper shell is `pointer-events: none`, only `.media-resize-clamp` handles capture; images/audio use click-to-lock. **Toolbar gap:** `isInsideMediaControlsUI` + `scheduleHoverHide` keep the floating toolbar open while the pointer crosses the gripper↔toolbar gap (no separate `wireToolbarHoverBridge` helper). **Toolbar reopen:** `existingToolbar` in `toolbar/mount.ts` sync-purges `[data-hm-closing]` siblings before remount — re-hover inside the 100ms exit fade otherwise stacks duplicate toolbars (`registry.cy.ts` guards this); cancel pending 100ms removal timers on rapid re-hover. **Column max-width:** gripper drag and inserts cap via `getEditorContentWidth` / `resolveEmbedLayoutDimensions`. **Pixel sync:** gripper commits attrs but node views must mirror width/height onto iframe/img/video/`.hm-media-host` via `sync*NodeLayout` in `src/loading/syncLayout.ts` (`syncElementPixelSize` on shell + surface). **Delete:** the document-CAPTURE-phase Delete/Backspace handler (`handleMediaDeleteKey`, registered with capture `true`) deletes the hovered media node (`deleteActiveMediaNode`), so it MUST bail when focus is inside editable chrome the node view owns — `INPUT`/`TEXTAREA`/`SELECT` inside `.media-toolbar` and `document.activeElement.closest('.hm-caption')`; capture fires before a nested element's bubble-phase `stopPropagation`, so the guard lives in the handler itself, not the caption/input listener. Node delete/cut must hide gripper + toolbar via `targetNodeCountChanged`, `appendTransaction`, and `purgeOrphanedResizeChrome()`. **Resize drag:** never inject overlay into node-view `contentDOM` — toggle `hypermultimedia__resize-gripper--dragging` on the gripper widget via `setGripperDragging` (`gripperDrag.ts`); use `setPointerCapture` + window pointer listeners so drag survives iframe overlays. **Drag end:** `restoreControlsAfterResize` clears `mediaResizing`, re-shows chrome only when `:hover`, calls `editor.commands.focus()` (never `blur()`). Gripper CSS: `src/styles/resize-gripper.css` (webapp `globals.scss` import; `--hm-resize-border` / `--hm-resize-handle-bg`). **Toolbar:** the media toolbar is a declarative action registry (`BASE_ACTIONS` / `NODE_ACTION_RECIPES`, resolved by doc-scoped `resolveMediaActions` in `toolbar/registry.ts`) mounted in the node's top-right corner (Notion-style) with an inline vs `…`-overflow split; overflow/submenu popovers use `@docs.plus/floating-popover`; CSS lives in `media-toolbar.css` (renamed from `media-toolbar-popover.css`, webapp `globals.scss` import updated); host integration uses three kit hooks: `mediaActions` (`composeMediaActions` lego builder + `layoutMediaActions` inline/overflow — no numeric `order`), `mediaToolbarIcons` (`resolveMediaToolbarIcon` with stable keys e.g. `align:${placement}`; one `getKitStorage` icons lookup per `renderMediaToolbar`), `mediaToolbar` (desktop mount factory; mobile returns null → webapp `getMediaToolbarFactory` opens `mediaControls` / `MediaControlsSheet`). Webapp wires Lucide via `createLucideToolbarIcons` + bounded `lucideSvgString.ts` cache; appends Comment via `getMediaActionsResolver` in `mediaComment.ts` (after Download → `CHAT_COMMENT`; same-heading re-comment must early-return when chat is already open). **Lifecycle/teardown:** `releaseToolbarTooltips` (registered on the bar in `renderMediaToolbar`) runs on close via `teardownToolbar` in `mount.ts` before deferred removal. `MediaResizeControls` plugin `destroy` should `controlsByEditor.delete(editor)`; close editor-carrying sheets including `mediaControls` on provider/document change so sheet payload does not hold a destroyed `Editor`. `isUploadedMedia` remains a separate kit hook. **Captions:** every media node has an editable `<figcaption>` whose `caption` attribute (shared logic in `src/caption.ts`: `captionAttribute`, `createCaptionElement`, `readCaption`, `wrapRenderWithCaption`, `mediaElementFrom`) is the source of truth and persists via collab/JSON/same-editor copy-paste; standalone-HTML `<figure>`/`<figcaption>` round-trip is image-only by design (embeds/video/audio keep the editable figcaption but never serialize `<figure>`). A node view with an editable figcaption MUST add `stopEvent` returning true when the event target is inside the caption element plus `ignoreMutation` for its subtree, or ProseMirror treats typing as editor transactions and deletes the node; caption commit/focus/keydown and `setMediaCaption` must guard on `editor.isEditable`, and the node-view `sync`/update re-sets the caption `contenteditable` to mirror it. **Node-type table:** classification is one table in `src/utils/media-target.ts` — `mediaKind(nodeType): 'asset' | 'embed'` (asset = image/audio/video, downloadable), with `isDownloadableMedia` and `isInteractiveEmbed` (= embed OR `video`) derived from it; keep the explicit `if (!nodeType) return false` null guard because `mediaKind(null)` returns `'embed'`. `toolbar/handlers.ts` `isDownloadable` / `isEmbedNode` derive from the same table, and video/audio node-view `update()` remount guards use the keyed `embedAttrsEqual(attrs, snapshot, KEYS)` helper from `embedKit.ts`, same as x/iframe.
- **`extension-hypermultimedia` detection API + webapp media-insert surface.** The extension root-exports `detectMediaType(url): MediaNodeType | null` (composed from the per-node validators in provider→image→generic-video/audio order), `isVideoUrl`/`isAudioUrl`, `parseYoutubeVideoId`, and the per-node validators. **`isImageUrl` is extension-STRICT** (only image-extension URLs / `data:image/` / `blob:`), which is _why_ `isMediaUrl` (paste-autoconvert predicate) deliberately EXCLUDES raw video/audio AND the webapp insert detector keeps a `?? 'image'` fallback for extension-less CDN image URLs — do not "fix" either. **Webapp insert UI** lives in `apps/webapp/src/components/TipTap/mediaPopovers/`: `MEDIA_INSERT_REGISTRY` (typed `Record<MediaNodeType, {label, Icon, insert, preview?, unfurl?}>`) is the single node→{command,UI} table; the headless `useMediaInsert(editor, {onInserted})` hook owns the tab/url/inserting form state + submit handlers + reads `docMetadata`; the desktop popover `MediaInsertPanel` and the mobile `mediaInsert`-sheet `MediaInsertSheet` are thin shells over it — do NOT merge them or re-inline the state. Previews: image/youtube static thumbnail (`MediaUrlPreview`; youtube via `i.ytimg.com/vi/<id>/hqdefault.jpg`), video/audio native `<video>`/`<audio>`, vimeo/soundcloud/loom metadata **unfurl card** via `useMediaUrlUnfurl` (debounced/cache-first/abort, reusing `hyperlinkPopovers/fetchMetadata` — the canonical metadata client; do not duplicate it). **Embeds render only with two host-side allowances:** (1) each iframe sets `referrerpolicy="strict-origin-when-cross-origin"` (in `iframeEmbedNode.ts` node-view + `renderIframeEmbedHTML`) because the webapp's `Referrer-Policy: same-origin` (`next.config.js`) strips the cross-origin `Referer` and YouTube then renders "player configuration error / 153"; (2) CSP `frame-src` / script / connect host lists live in `apps/webapp/config/security/third-party-hosts.js` (wired through `next.config.js` — see Learned Workspace Facts bullet below). oEmbed fetches `publish.x.com` directly (`helper.ts`). **The resize-gripper overlay forces `margin: 0 !important`** (`resize-gripper.css`): it is a direct `.ProseMirror` child and inherits the host content-flow margin (~12px), which shifts the overlay off the media box and desyncs `offsetTop`↔`style.top` (breaking drag); `positionResizeGripper` measures `.hm-media-host` via `getBoundingClientRect`, and `activateResizeGripper` adds `--active` before measuring (offsetParent is null while `display:none`).
- **Webapp third-party hosts (CSP + Workbox).** Single source of truth: `apps/webapp/config/security/third-party-hosts.js` — consumed by `next.config.js` CSP headers and `config/pwa/workbox-runtime-caching.js`. X `widgets.js` is fetched via the PWA service worker; `connect-src` must list `platform.twitter.com`/`platform.x.com`, not only `script-src`. GA4: include `analytics.google.com` in `connect-src`. `config/pwa/workbox-runtime-caching.js` wraps `next-pwa/cache` to bypass third-party embed/analytics hosts from Workbox runtime routes — otherwise failed fetches (CSP gaps, ad blockers) surface uncaught `no-response` promises; page-level GTM failures use `swallowOptionalScriptError` in `GoogleAnalytics.tsx`. YouTube `doubleclick`/`log_event` and SoundCloud `dwt.soundcloud.com` console errors run inside embed iframes and are usually ad-blocker noise when players still work.
- **Extension clean-room Cypress harnesses** (all five `@docs.plus/extension-*`): specs under `cypress/e2e/**` and `cypress/docs/**` (kebab-case dirs); `bun run test` uses `start-server-and-test` against `docs-playground` (ports 5173–5177; `pretest` builds `dist/`). **Mod shortcuts:** never hard-code `Meta` for TipTap `Mod` bindings in Cypress — on Linux CI `Mod` is Ctrl; mirror `inline-code` (`Cypress.platform === 'darwin' ? { metaKey: true } : { ctrlKey: true }`) or hyperlink's shared `cy.pressModK()` (must `.focus()` `#editor [contenteditable="true"]`, not `.click()`, so a prior `selectText()` range survives). **Hyperlink popover waits:** `.floating-popover` stays `opacity: 0` until `.visible` (next rAF); wait via `cy.getVisibleFloatingPopover()` before asserting inner popover content with `be.visible`. **`scroll-stickiness.cy.ts` create test:** `selectText()` scrolls the selection to the viewport bottom so floating-ui `flip()` can swap placement mid-scroll and break equal-Δtop assertions — call `pinSelectionNearViewportTop()` (~120px headroom) after `selectText` before Mod+K. **CI Cypress binary:** npm `cypress` and the Linux runner binary are separate (`~/.cache/Cypress`, not `node_modules`). `.github/actions/setup-cypress` must cache `~/.cache/Cypress` and run `bunx cypress install` from an extension workspace dir (e.g. `extensions/extension-hyperlink`) — repo-root `bunx` resolves `cypress@latest` while suites run the catalog-pinned version. **CI build cache:** `.github/actions/build-extensions` must cache `packages/floating-popover/dist` + `packages/floating-tooltip/dist` alongside `extensions/*/dist` (key `ext-v3-*`; hashFiles includes `tsup.base.ts`, `tsconfig.base.json`, and `scripts/build-extensions.sh`); cache hits that skip the build without floating `dist` break typecheck and hyperlink unit tests. **`extension-tests` path filter:** gated by a `changes` job (`dorny/paths-filter`; paths in `.github/filters/extensions.yaml`, keep in sync with `build-extensions` cache); `build` may treat a skipped `extension-tests` as pass only when `changes` succeeded and nothing extension-related changed — fail-closed if `changes` fails (skipped tests are not a pass). A deploy commit `(build): front back` with zero file changes can still run extension-tests when the filter compares against the prior deploy commit range. **Deploy triggers:** `.github/scripts/evaluate-deploy-trigger.sh` parses commit messages — `prod` mode matches `(build): front|back|front back`; `uptime-kuma` mode matches `(build): uptime-kuma` (anchored regex, not loose `contains()`). **Husky `pre-push` / `scripts/hooks/pre-push.sh`:** runs `bun run check` (lint + format + styles + types) and selective extension **builds** only — NOT extension Cypress suites; run `bash scripts/run-tests.sh --extensions` locally before pushing extension spec changes. Extension change detection greps `extensions/${ext}/` for per-package builds and runs full `scripts/build-extensions.sh` when shared dist inputs change (`floating-*`, `tsup.base.ts`, `tsconfig.base.json`, `build-extensions.sh`, `publishable-extensions.ts`, `bun.lock`) — mirror the dist subset in `.github/filters/extensions.yaml`. `setup-bun` alone is not enough on Linux runners (`not-installed-ci-error`). Each extension's `cypress/tsconfig.json` extends `@docs.plus/playground/cypress/tsconfig.json` with `include: ["./**/*.ts"]` — without it, tsc inherits the package `rootDir: src` and fails once `test/playground/tsconfig.json` pulls playground outside `src/`. Publishable extensions declare `engines.node` only (aligned with root floor) — not `engines.bun`; they ship prebuilt browser bundles and peers are `@tiptap/*`, while Bun stays the root maintainer publish/install toolchain. README galleries: hero + optional `<details>`/`<summary>` scenes with light/dark `<picture>` pairs; `docs:screenshots` runs `cypress/docs/*`; `flattenReadmeScreenshots.ts` + shared `readmeGallery.ts` flatten into tracked `assets/*.png` (hypermultimedia: 18 PNGs, one per node type). HM asset nodes use committed CC0 media in `assets/readme-media/` served by playground `--readme-media-dir` at `/readme-media/*` (not npm tarball). Gallery captures wait for media ready, then `hoverMediaControls`/`activateImageGripper` so toolbar + gripper appear in frame (X: toolbar only, ~6s `widgets.js` settle). Family harness index: `extensions/README.md`; CI/local dist build: `scripts/build-extensions.sh` (`.github/actions/build-extensions`); `run-tests.sh --extensions` with `EXTENSION_DIST_READY=1` skips per-package `pretest` rebuilds; all-five publish gate: `scripts/extension-preflight.sh`. Publishable dir list + gate metadata: `scripts/publishable-extensions.ts` (`hasUnit` drives Jest vs Cypress-only notes; imported by `release-family.ts`). Extension `.gitignore` must anchor package-root docs ignore as `/docs/`, not `docs/`, or `cypress/docs/*.cy.ts` is accidentally ignored. Publish lifecycle hooks must use `bunx release-prepack` / `bunx release-preflight` — bare bin names fail under `bun publish` (workspace `.bin` not on PATH). Phase 1 laptop publish: `bun publish --tag next --otp …` only — never `npm publish` (packed `peerDependencies` stay `catalog:` and break external installs). Internal workspace deps use `workspace:*`, not `file:` paths. `extension-hypermultimedia` playground imports built `@docs.plus/extension-hyperlink`; HM `pretest`/`docs:screenshots` builds hyperlink first. Hypermultimedia helpers `activateImageGripper` / `hoverMediaControls` / `dragResizeClamp` / `expectMediaLoadingPending|Ready` live in `cypress/support/e2e.ts`; resize specs assert rendered inline size on `img`/iframe/video/`.hm-media-host`, not attrs alone; caption specs use caret-based backspaces — not `{selectall}` or `{moveToStart}`. Clear stale playground ports with `lsof -ti:5173,5174,5175,5176,5177 | xargs kill -9`.
