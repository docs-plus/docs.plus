...
- Do not call URL, router-adjacent, floating-menu, or landing mobile-keyboard UI production-ready or “ship” until the real interaction path is checked in the browser (light + dark); on `/`, verify hero+card vertical centering with keyboard closed and compact repositioning when the slug input is focused on iOS Safari and Android Chrome. `history.pushState` can leave `h`/`id` in the bar where stripping only bookmark-style params no-ops — context menus need pointer cursor, hover contrast, and full-width dividers verified the same way.
...
- **`extension-hypermultimedia` (embed + resize + insert).** Floating toolbars via `@docs.plus/floating-popover`; per-host embed attrs in each `embedOptions.ts`; shared iframe helpers in `iframeEmbedNode.ts` + `embedKit.ts`. Desktop resize chrome is Notion-style hover (`MediaResizeControls`, `WeakMap` per editor, gripper `pointer-events: none` on interactive embeds). Toolbar is a declarative registry (`resolveMediaActions`, `existingToolbar` purges `[data-hm-closing]` on re-hover). Root exports `detectMediaType` / per-node validators — **`isImageUrl` is extension-STRICT**; webapp paste/insert keeps `?? 'image'` for extension-less CDN URLs. Pad insert UI: `TipTap/mediaPopovers/` (`MEDIA_INSERT_REGISTRY`, `useMediaInsert`, separate desktop popover vs mobile sheet). Iframe embeds need `referrerpolicy="strict-origin-when-cross-origin"` (webapp `Referrer-Policy: same-origin`); CSP + Workbox host lists live in `config/security/third-party-hosts.js` (also GA4/X connect-src). Gripper overlay uses `margin: 0 !important` on direct `.ProseMirror` children. Full node-view/caption/CI detail stays in main `AGENTS.md` §Extension Workflow and package READMEs.
