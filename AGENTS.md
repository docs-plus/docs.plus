...
- **`git push` runs that production build for you, so the rule above covers builds you never typed.** `scripts/hooks/pre-push.sh` calls `bun run --filter @docs.plus/webapp build:ci` whenever the unpushed range touches `apps/webapp/**`. That build writes a production `BUILD_ID` into the same `apps/webapp/.next` a live `next dev` owns, and every route then answers a bare `Internal Server Error`. The hook redirects the build output to `/dev/null`, so nothing connects the push to the break. It has bitten twice. The 2026-06-28 build failed early and only aborted the push. The 2026-08-19 build succeeded and corrupted the running server, and a Supabase restart got the blame first. Stop the webapp dev server before you push. To diagnose, run `ls apps/webapp/.next`: a `BUILD_ID` or `prerender-manifest.json` there means a production build ran. To recover, run `rm -rf apps/webapp/.next` **and** restart the dev server, because the delete alone does not fix a server that keeps running. Never reach for `git push --no-verify`.
