...
- Large multi-agent chatroom/webapp cleanups need supervisor review of the plan and resulting diff before and after execution, not straight-through commits.
...
- For large HoE-style reviews (chatroom, TOC, sheets, composer), build cohesive context from related modules first, then wait for the explicit task before implementing.
- For large mobile UI plans, use a Cursor canvas mockup to preview end-state UX before cold plan execution when the user asks how it will look.
...
- Exact semver pins without `^` in a workspace `package.json` (e.g. webapp `next: "15.5.9"`) are often skipped by `bun run update` / `--target compatible`; bump with `bun update <pkg> --latest --filter <workspace>` or edit the pin, then rely on root `catalog` + `bun.lock` for catalog deps.
- MessageComposer composer bar: format toggle sits left of the input (`ToggleToolbarButton` / `Icons.textFormat`); emoji, mention, and send are ungrouped ghost controls on the right with inline Lucide `@icons` className literals in each action-button file (shared size/stroke)—not mixed Material/Remix/Ionic icons.
- Mobile chatroom composer: `submitOnEnter: false` in `MessageComposer.tsx` — keyboard Enter inserts a newline; only the Send button submits. `keepKeyboardAfterSubmit: isMobile` in `useComposerSubmit` refocuses after send so the keyboard stays up for rapid follow-up messages.
- Chat + pad hyperlink UI: `getHyperlinkPopoverConfig(isMobile)` in `TipTap/hyperlinkPopovers/` — desktop uses `createHyperlinkDesktop` + a single page-level `HyperlinkPopoverPortal` (never mount the portal inside `MessageComposer` when `DesktopEditor` already mounts it); mobile uses `createHyperlinkMobile` → `linkEditor` sheet via `useSheetStore`. Centered mobile modals (e.g. planned composer link dialog) must anchor to `--visual-viewport-height` / `--visual-viewport-offset-top` from `syncVisualViewportToCssVars`, not `fixed inset-0` centering, or iOS keyboard covers the bottom half.
- Chatroom `BottomSheet` (chatroom sheet only for now): on composer `focusin`, `isKeyboardOpen`, or `useComposerEmojiPanelStore.isOpen`, snap to full height via `sheetRef.snapTo(CHATROOM_TOP_SNAP_INDEX)` plus `avoidKeyboard` on all mobile sheets. The composer emoji panel must OR into the same trigger so opening it from a peek-state chatroom expands the sheet to full.
- Tail unread suppression: when the open chatroom is at bottom and the loaded window includes tail (`dataIncludesTailRef`), `ChatroomContext.onAtBottomChange` sets `unreadSuppressedChannelId`; `useUnreadCount` returns 0 and channel/optimistic writes clamp so TOC, pad title, and `JumpToPresentButton` show no phantom badge until the user scrolls up.
- MessageComposer layouts: `ComposerLayout` picks mobile vs desktop; `ComposerDesktopLayout` / `ComposerMobileLayout` wrap `ComposerBar`. `ChannelComposer` renders `MsgComposer.ComposerLayout`. TipTap body stays `MessageComposer.EditorContent`. Old names `*Chrome` and `MessageComposer.Editor` (layout switch) are retired — see §Code Quality component naming.
