...
- **ChatMediaGallery UX (Discord parity).** Full-bleed lightbox (`modalBackdropHeavyClassName`, `z-[100]`, `h-[100dvh]`, `overscroll-none`): top-left author block (name line 1, sent-at line 2); top-right grouped action pill (zoom on image slides only, reply when `originMessage`, download, open, ⋯) plus separate close; copy link lives in overflow only; `n / total` counter when multi-item. Tap letterbox/stage (not controls) toggles the overlay controls when not zoomed (`controlsVisible`, `MOTION_PANEL_MS` fade). Spoiler feed tiles stay **two-tap reveal** before `openGallery` — carousel must not bypass feed spoiler state.
...
- **Gallery download / open / copy-image.** `galleryMediaActions.ts`: `saveMediaFile`, `openMediaLink`, `copyMediaImage` toast on every outcome. iOS Safari: download → `navigator.share({ files })` when `canShare`, else open tab + Info toast; copy image → clipboard then share fallback. Desktop download uses anchor + Success toast.
- **Gallery image zoom.** `useGalleryZoomPan` + store `requestZoomIn` / `requestZoomReset`: toolbar toggles fit↔zoom; desktop click/dblclick/wheel; iOS double-**tap** (native `touchend`, 300ms/30px slop) and pinch (native `touchmove` `{ passive: false }` on host — React synthetic `preventDefault` is ignored on iOS). `touch-none`/`overscroll-none` on the zoom host **only when zoomed** so horizontal carousel swipe works at fit. Carousel viewport: `touch-pan-x touch-pinch-zoom` at fit, `touch-none` when `slideZoomed`; carousel ignores multi-touch/pinch. Disable Floating UI `escapeKey` dismiss — first **Escape** resets zoom when zoomed, second closes. Mobile toolbar targets: 44px via `settings.editor.isMobile`.
