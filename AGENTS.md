...
- **Draft vs mode memory.** Text/HTML drafts live only in IndexedDB (`messageComposerDB` via `syncComposerDraft` / `discardComposerDraft`); Zustand `*MessageMemory` holds reply/edit/comment/forward UI modes only — do not reintroduce `messageDraftMemory`.
