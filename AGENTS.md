...
- **Extension clean-room Cypress harnesses** (all five `@docs.plus/extension-*`): ports 5173–5177 via `docs-playground`; `pretest` builds `dist/`; Linux CI uses Ctrl for TipTap `Mod` (never hard-code `Meta`); wait on `.floating-popover.visible` before inner assertions; `.github/actions/build-extensions` must cache `floating-popover` + `floating-tooltip` dist with extensions. Shared markdown E2E helpers live in `@docs.plus/playground/cypress/markdownCommands.ts` (`registerPlaygroundMarkdownCommands`; webapp imports via `cypress/support/editor/markdownCommands.ts` with `ensureTitleDocumentH1` for TitleDocument). Hypermultimedia markdown hooks: `typedMediaMarkdown.ts` wired per kit node. Full ports, CI guards, README galleries, and publish hooks: `extensions/README.md` and main `AGENTS.md` §Tests / §Publishing.
