...
- **TOC resize** (`useTocResize.tsx`, `ResizeHandle.tsx`, `SlugPageLoader.tsx`): max width `TOC_MAX_WIDTH_RATIO = 0.46` of `.editor` row; clamp on drag + window resize. Vertical sash mounts on the `.editor` row (not inside the TOC column) at `left: tocWidth`, `z-50`, straddling the split (`right-[calc(var(--resize-sash-hit)/-2)]` on a zero-width anchor) so it stays the **sole** continuous hairline above the docked chat (`z-40`) — do not nest it under TOC `isolate` or chat paints over it and the panel edge reads as a thicker, offset second line. Always draws a 1px idle hairline (`--resize-sash-idle`); widens/recolors to primary on hover/drag. Do not reintroduce column `border-r` or idle `after:opacity-0`. Horizontal chat sash: **`bottom-full`** on the panel top edge — never `top-0 -translate-y-1/2` (overlaps chat toolbar and steals clicks).
...
- TOC uses React `UnreadBadge` only. Shared count math is `resolveUnreadCount` (`utils/unreadDisplay.ts`); ProseMirror widgets sync via `syncHeadingWidgetUnread` (`.ha-chat-btn` only — never TOC `.toc__chat-trigger`).
...
- **TOC row = daisyUI menu item.** `ul.toc__list.menu` owns padding / hover / nest rails (stock — no TOC `::before` strengthen); desktop doc-title is the sticky first `<li class="toc__header">` wrapping the same `TocRow` as headings. Chat-open → `menu-active`, scroll-spy → `menu-focus` (not custom accent bars / `bg-base-300`). `TocRow` is three grid children (leading | title `<a>` | `TocRowTrail`) — do not reintroduce a flex override or absolute trail + spacer math. Fold is a plain `<button>` chevron (not `btn-ghost`). Grip / fold / chat are always visible. Drag overlay SCSS: `_tocDrag.scss`. TOC `ScrollArea` uses `preserveWidth={false}` + `hideScrollbar`.
...
- TOC chat/unread live in the menu item end slot (`TocRowTrail`); presence may shift chat x — do not reintroduce absolute trail + spacer width math.
