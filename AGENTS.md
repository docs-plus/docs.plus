...
- **Production observability + landing shell + PWA.** Observability stack in `docker-compose.observability.yml` (Grafana/GlitchTip/Telegram; add `GLITCHTIP_DSN` + `NEXT_PUBLIC_GLITCHTIP_DSN` to prod `.env` after bootstrap). Deploy commit subject `(build): <domains>` — canonical order `back → front → observability → uptime-kuma` (`.github/WORKFLOWS.md`). Landing `/` is SSG with deferred anon auth via `routePolicy.ts`; `document-styles.scss` dynamic-import only when `documentShell`; mobile home keyboard uses `useHomeKeyboardCompact` + visual viewport sync on slug focus. **PWA Workbox + CSP:** service-worker `fetch()` for cross-origin images/audio/video is gated by `connect-src`, not `img-src`/`media-src` — default next-pwa rules can break any remote avatar/embed URL despite `img-src https:`. Fix in `config/pwa/workbox-runtime-caching.js` only: same-origin-only for extension rules 3/5/6, skip `image`/`audio`/`video` on cross-origin NetworkFirst rule 13 — do not enumerate OAuth/media CDNs in `connect-src`. Workbox `urlPattern` matchers must be self-contained (next-pwa serializes via `Function.toString()` — no module-scope refs). Rebuild webapp for `sw.js` changes.
