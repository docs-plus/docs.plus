...
- After substantive chatroom/webapp, Supabase, or extension-package work (playground harness, hypermultimedia), expect a cohesive DRY/KISS pass (dead-file import-graph sweep, redundant store fields, code-simplifier, thermo-nuclear review when invoked) before treating the work as production-ready—not optional follow-up polish.
...
- **Extension clean-room Cypress harnesses** (`extension-hyperlink`, `extension-hypermultimedia`): specs under `cypress/e2e/**` (kebab-case dirs); `bun run test` uses `start-server-and-test` against `docs-playground` (hyperlink 5173, hypermultimedia 5174; `pretest` builds `dist/`). Each extension's `cypress/tsconfig.json` extends `@docs.plus/playground/cypress/tsconfig.json` with `include: ["./**/*.ts"]` — without it, tsc inherits the package `rootDir: src` and fails once `test/playground/tsconfig.json` pulls playground outside `src/`. Hypermultimedia helpers `activateImageGripper` / `hoverMediaControls` / `dragResizeClamp` / `expectMediaLoadingPending|Ready` live in `cypress/support/e2e.ts`. Resize specs assert rendered inline size on `img`/iframe/video/`.hm-media-host`, not attrs alone; caption specs use caret-based backspaces — not `{selectall}` or `{moveToStart}`. Start `bun run playground` as a managed background process; clear stale ports with `lsof -ti:5173,5174 | xargs kill -9`.
