...
- **`extension-hypermultimedia` 2.1.0 detection API + webapp media-insert surface.** The extension root-exports `detectMediaType(url): MediaNodeType | null` (composed from the per-node validators in provider→image→generic-video/audio order), `isVideoUrl`/`isAudioUrl`, `parseYoutubeVideoId`, and the per-node validators. **`isImageUrl` is extension-STRICT** (only image-extension URLs / `data:image/` / `blob:`), which is _why_ `isMediaUrl` (paste-autoconvert predicate) deliberately EXCLUDES raw video/audio AND the webapp insert detector keeps a `?? 'image'` fallback for extension-less CDN image URLs — do not "fix" either. **Webapp insert UI** lives in `apps/webapp/src/components/TipTap/mediaPopovers/`: `MEDIA_INSERT_REGISTRY` (typed `Record<MediaNodeType, {label, Icon, insert, preview?, unfurl?}>`) is the single node→{command,UI} table; the headless `useMediaInsert(editor, {onInserted})` hook owns the tab/url/inserting form state + submit handlers + reads `docMetadata`; the desktop popover `MediaInsertPanel` and the mobile `mediaInsert`-sheet `MediaInsertSheet` are thin shells over it — do NOT merge them or re-inline the state. Previews: image/youtube static thumbnail (`MediaUrlPreview`; youtube via `i.ytimg.com/vi/<id>/hqdefault.jpg`), video/audio native `<video>`/`<audio>`, vimeo/soundcloud/loom metadata **unfurl card** via `useMediaUrlUnfurl` (debounced/cache-first/abort, reusing `hyperlinkPopovers/fetchMetadata` — the canonical metadata client; do not duplicate it). **Embeds render only with two host-side allowances:** (1) each iframe sets `referrerpolicy="strict-origin-when-cross-origin"` (in `iframeEmbedNode.ts` node-view + `renderIframeEmbedHTML`) because the webapp's `Referrer-Policy: same-origin` (`next.config.js`) strips the cross-origin `Referer` and YouTube then renders "player configuration error / 153"; (2) `next.config.js` `frame-src` must list `www.youtube.com`/`www.youtube-nocookie.com`/`player.vimeo.com`/`w.soundcloud.com`/`www.loom.com`. **X embeds** need a deliberate Twitter-JS allowlist in `next.config.js`: `script-src platform.twitter.com`/`platform.x.com`, `connect-src publish.x.com`/`publish.twitter.com`/`syndication.twitter.com`/`syndication.x.com`, `frame-src platform.twitter.com`/`platform.x.com`/`syndication.twitter.com`/`syndication.x.com`; oEmbed fetches `publish.x.com` directly (`helper.ts`). **The resize-gripper overlay forces `margin: 0 !important`** (`resize-gripper.css`): it is a direct `.ProseMirror` child and inherits the host content-flow margin (~12px), which shifts the overlay off the media box and desyncs `offsetTop`↔`style.top` (breaking drag); `positionResizeGripper` measures `.hm-media-host` via `getBoundingClientRect`, and `activateResizeGripper` adds `--active` before measuring (offsetParent is null while `display:none`).
