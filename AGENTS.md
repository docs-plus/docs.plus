...
- Implementation plans under `docs/superpowers/plans/` must stay cold-execution ready: sync with landed code, mark completed tasks, include explicit anti-over-engineering guardrails, and remove or correct stale steps that would mislead an executing agent.
...
- Reply and comment reference blocks in the feed are actionable `<button>` jumps (`ReplyReference`, `CommentReference`; `data-key="reply-ref-{id}"` etc.) calling `ChatroomContext.scrollToMessage` (`useScrollToMessage`: in-window `scrollToItem` or RPC `fetch_message_window` replace + `useMessageHighlight` flash). Comment jumps require `metadata.comment.heading_id`; legacy rows without it stay static.
