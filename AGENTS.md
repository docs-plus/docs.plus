...
- **Themes are token sets** (daisyUI blocks in `globals.scss`): 3 base (`docsplus` / `docsplus-dark` / `docsplus-dark-hc`) + 2 opt-in premium light/dark pairs (`docsplus-graphite[-dark]`, `docsplus-paper[-dark]`), added alongside — never replacing — the base. Follow design-system.md §Themes → **Adding a theme**: a new _dark_ theme MUST also be added to the `@custom-variant dark` matcher, the shared dark semantic-override block (+ its own `--pad-well`/`--color-media-*`), and the `em-emoji-picker` + `.edit-fab` dark selectors, or every `dark:` utility silently falls back to light; a new _light_ theme joins `LIGHT_RESOLVED_THEMES` (`themeStore.ts`) — the single "is-it-light" source read by `getEmojiMartTheme` and the emoji `Picker`. `resolveTheme`/`PREFERENCE_TO_THEME` and the `_document.tsx` FOUC script both map preference→theme (keep in sync). `System` resolves to the base light/dark pair only, never a premium theme.
...
- Multi-image collage: up to **4** tiles + `+N` via `getChatMediaCollageLayout` (Telegram/Discord layouts, `object-cover` cells). Unified lightbox: root `ChatMediaGallery` via `chatMediaGalleryStore.openGallery(medias, at?)` — one message-scoped carousel across `image`, `video`, and `audio` in feed visual order (`[...images, ...videoAndAudio]` from `galleryPlaylist.ts`; `file` stays inline). Signed-URL failures surface `MediaUnavailable` + retry. Reply/edit bars use `ContextBarMediaThumb`.
