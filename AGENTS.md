...
- **Document comments.** First-class `messages.type = 'comment'` with `metadata.comment` holding `CommentAnchorV1` (`v:1`, `text`|`media` kinds; types in `types/comment.ts`, anchor helpers in `services/commentAnchor.ts`, preview in `utils/commentPreview.ts` with TipTap adapter in `mediaPopovers/buildCommentPreview.ts`). Send via `sendCommentMessage`; composer draft is `CommentMessageMemory` (`Profile | null` user). `publishDocumentComment` → `CHAT_COMMENT` (`services/chatEvents.ts`) → `openCommentComposer` in `services/openHeadingChatroom.ts` (distinct from `CHAT_OPEN` browse via `openHeadingChatBrowse`). Feed/composer chrome: shared `ReferenceJumpButton` — reply=`border-l-info`+`Icons.reply`, comment=`border-l-secondary`+`Icons.comment`; media preview via shared `CommentPreviewVisual` at `components/CommentPreviewVisual.tsx`; `CommentAnchorPreview` stacks thumbnail above label/excerpt; feed cards stay compact and non-interactive (no inline players in Virtuoso).
