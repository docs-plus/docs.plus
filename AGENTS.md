...
- MessageComposer composer bar: format toggle sits left of the input (`ToggleToolbarButton` / `Icons.textFormat`); emoji, mention, and send are ungrouped ghost controls on the right via `composerActionClasses.ts` (Lucide `@icons`, shared size/stroke)—not mixed Material/Remix/Ionic icons.
- Chat + pad hyperlink UI: `getHyperlinkPopoverConfig(isMobile)` in `TipTap/hyperlinkPopovers/` — desktop uses `createHyperlinkDesktop` + a single page-level `HyperlinkPopoverPortal` (never mount the portal inside `MessageComposer` when `DesktopEditor` already mounts it); mobile uses `createHyperlinkMobile` → `linkEditor` sheet via `useSheetStore`.
