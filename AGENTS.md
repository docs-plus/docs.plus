...
- Extension markdown cohesion means hyperlink plus every hypermultimedia kit node (all nine, including Spotify) with clean-room round-trip Cypress E2E for **both markdown and HTML** (`getHTML()` → `setContent()`) — do not call import/export support complete when any node or serialization path is still uncovered; when fixing one node's copy/paste, add or extend the shared HTML round-trip spec for all kit nodes, not only the broken one.
