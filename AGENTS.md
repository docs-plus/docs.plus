...
- **Anon visitors get read-only PUBLIC chat.** `useChannelInitialData.processChannelData` and the chatroom open path must skip `upsertChannel`, `joinChannel`, `mark_messages_as_read`, and message INSERTs when `auth.uid()` is null; RLS will deny them anyway and the FE 403 toast is worse UX than a clean read-only render. Authenticated writes (composer, retry) assume the row exists; gate the write entry points themselves, do not gate inside the RPCs.
