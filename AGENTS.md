...
- Extension markdown cohesion means hyperlink plus every hypermultimedia kit node (all eight) with clean-room round-trip Cypress E2E — do not call markdown support complete when any node or import/export path is still uncovered.
...
- **Production observability.** Stack in `docker-compose.observability.yml`; server env file `.env.observability` needs `GRAFANA_ADMIN_PASSWORD`, `GLITCHTIP_SECRET_KEY`, `GLITCHTIP_POSTGRES_PASSWORD`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, and main-app `DATABASE_URL` (postgres-exporter — not GlitchTip's DB). After GlitchTip bootstrap, add matching `GLITCHTIP_DSN` + `NEXT_PUBLIC_GLITCHTIP_DSN` to prod `.env` for webapp/hocuspocus OTel. Deploy: `(build): <domains>` in the commit subject selects which domains deploy; canonical order is always `back → front → observability → uptime-kuma` (fail-fast) — see `.github/WORKFLOWS.md` for the grammar and truth table. `SENTRY_ROLE` stays in compose, not operator `.env`.
- **Landing page + app shell.** `/` is SSG (`pages/index.tsx`) with SSR `<Head>` from `homeMetadata.ts`; indexable home vs `noindex` slug pages. `utils/routePolicy.ts` gates `documentShell`, deferred anon auth on `/` (`ensureAnonymousSession` on navigate only), analytics, and `visualViewport` mode (`off`|`landing`|`document`). `_app` loads `globals.scss` + `shell.scss` always; `document-styles.scss` loads only via dynamic `import()` from `_app` when `documentShell` or on navigate prefetch — Next.js forbids global SCSS in components. `AppQueryClientRoot` dynamically imports `DocumentShellInner` (Virtuoso/FloatingTree/ChatMediaGallery). Slug helpers: `sanitizeDocumentSlug` / `randomDocumentSlug`. SEO: `pages/robots.txt.ts`, `pages/sitemap.xml.ts`. Mobile home keyboard: `useHomeKeyboardCompact` + `#home-slug-input` / `#home-action-block` — center hero+card when keyboard closed; compact top-aligned layout when keyboard open or viewport height drops below ~82% of layout height; `useVisualViewportCssSync({ mode: 'landing' })` scrolls `#home-action-block` into the visible viewport on slug focus.
