...
- **Title write.** One client module `apps/webapp/src/utils/titleWrite.ts`. Adapters stay adapters. Do not merge Title write with Access mutation. Do not invent a workspace package for the tag strip. Do not fold first-heading admission into this module. A signed-in rename posts `title_changed`. History paints the snapshot username. Chat paints the live username. The Documents-list adapter relays only when the renamed id is the open pad. Open document and first-edit ownership live in `CONTEXT.md` §Document access. See `CONTEXT.md` §Pad outline.
